    ...
    <v-container fluid>
      <v-card-title id="card-title">
        ...
        <div class="flex_row justify-lg-space-between">
          ...
          <v-btn class="tall__btn px-8" color="primary" @click="createDebitNote()" v-else>Create</v-btn>
        </div>
      </v-card-title>
  ...
  methods: {
    ...
    async createDebitNote() {
      try {
        ...
        this.invoiceObj['debitDate'] = new Date(Date.now())
        this.invoiceObj.type = 'credit note'
        this.invoiceObj['memo'] = ''
        this.invoiceObj['debit_balance'] = this.computedTotal
        this.invoiceObj['debit_amount'] = this.computedTotal
        ...
        await this.$axios
          ...
          this.$nuxt.$emit('record-payment-status', {
            success: true,
            message: 'A new debit note has been created successfully',
          })
          this.closeEstimate()
      }
      } catch (error) {
        this.$nuxt.$emit('record-payment-status', {
            success: false,
            message: 'Failed to create debit note for Invoice',
          })
      }
      finally {
