                    ...
                    <v-row class="pt-4">
                        <v-col class="" cols="12" sm="12" md="6">
                            <p class="grey-heading-text font-weight-medium textFontSize"><span
                                    >Select Passport Release Purpose</span> <span
                                    class="red--text">*</span></p>
                            <v-select dense :items="passportReleasedPurpose"
                                v-model="selectedPassportReleasePurpose"></v-select>
                        </v-col>
                    </v-row>
                    <v-row class="pt-4">
                        <v-col cols="12" sm="6" md="6">
                            <v-menu v-model="menu1" :close-on-content-click="false" :nudge-right="40"
                                ...
                                <template v-slot:activator="{ on, attrs }">
                                    <v-text-field v-model="passport_release_Date" label="Passport Release Date"
                                        prepend-icon="mdi-calendar" readonly v-bind="attrs" v-on="on"></v-text-field>
                                </template>
                                <v-date-picker v-model="passport_release_Date" @input="menu1 = false"></v-date-picker>
                            </v-menu>
                        </v-col>
                        ...
                        <v-col cols="12" sm="6" md="6">
                            <v-menu v-model="menu2" :close-on-content-click="false" :nudge-right="40"
                                transition="scale-transition" offset-y min-width="auto">
                                <template v-slot:activator="{ on, attrs }">
                                    <v-text-field v-model="passport_return_Date" label="Passport Return Date"
                                        prepend-icon="mdi-calendar" readonly v-bind="attrs" v-on="on"></v-text-field>
                                </template>
                                <v-date-picker v-model="passport_return_Date" @input="menu2 = false"></v-date-picker>
                            </v-menu>
                        </v-col>
                    </v-row>
                    <v-row class="pt-4">
                        <v-col class="" cols="12" sm="12" md="12">
                            <p class="grey-heading-text font-weight-medium textFontSize"><span
                                    >Description</span> <span class="red--text"
                                    v-if="selectedPassportReleasePurpose == 'Others'">*</span></p>
                            <v-textarea dense rows="2" v-model="request.letter_fields.description" class="redTextForm"
                                :rules="selectedPassportReleasePurpose == 'Others' ? genericRule : []"></v-textarea>
                        </v-col>
                    </v-row>
                    <v-row>
                        <v-col class="text-right">
                            <v-btn dark color="primary" @click.prevent="createPassportReleaseRequest()"
...
export default {
    ...
    data() {
        return {
            passportReleasedPurpose: ["Annual Leave", "Travel Visa Application", "Bank Requirement", "Passport Renewal", "Others"],
            selectedPassportReleasePurpose: '',
            passport_release_Date: (new Date(Date.now() - (new Date()).getTimezoneOffset() * 60000)).toISOString().substr(0, 10),
            passport_return_Date: (new Date(Date.now() - (new Date()).getTimezoneOffset() * 60000)).toISOString().substr(0, 10),
            menu1: '',
            menu2: '',
    ...
    methods: {
        async createPassportReleaseRequest() {
            if (this.$refs.form.validate()) {
                ...
                let body = {
                    purpose: this.selectedPassportReleasePurpose,
                    passport_return_Date: this.passport_return_Date,
                    passport_release_Date: this.passport_release_Date,
                ...
                let apply_passport_request = await this.$axios.$post("/passport/release_request", body, { headers: { Authorization: AuthStr } })
                ...
                if (apply_passport_request && apply_passport_request.success) {
                    ...
                    this.selectedPassportReleasePurpose = ''
                    this.passport_return_Date = ''
                    this.passport_release_Date = "",
