<template>
  <div class="tw-space-y-4 tw-h-full tw-flex tw-flex-col tw-max-h-120">
    <!-- Header -->
    ...
    <div v-if="loading && emailHistory.length === 0" class="tw-flex-1 tw-overflow-y-auto tw-pr-2 tw-max-h-104">
    ...
    <!-- No Emails State -->
    <div v-else-if="emailHistory.length === 0 && !loading" class="tw-text-center tw-py-8 tw-flex-1 tw-max-h-104">
      <v-icon color="gray" size="48" class="tw-mb-4">mdi-email-outline</v-icon>
      <p class="tw-text-gray-500 tw-text-lg">No emails sent to this employee</p>
      <p class="tw-text-gray-400 tw-text-sm tw-mt-2">Email history will appear here once emails are sent</p>
    </div>

    <!-- Email List -->
    <div v-else class="tw-flex-1 tw-overflow-y-auto tw-pr-2 tw-max-h-104">
      <div class="tw-space-y-3">
        <v-expansion-panels>
          <v-expansion-panel
...
<script>
export default {
  name: 'InternalEmployeeEmailsLog',
  props: {
    employeeEmail: {
      type: String,
      default: ''
