                ...
                <v-card class="rounded-xl allRequests" style="box-shadow: 0px  24px 30px #959EA51A;" min-height="720px"
                    ...
                    <v-row class="mx-0" style="max-width:100%" justify="center">
                        <v-col cols="12" class="pa-0 ">
                            <v-card-title>
                                <v-select :items="computeRequestTypeList()" placeholder="Select Request Type"
                                    v-model="requestType" item-text="name" item-value="value" dense outlined
                                    ...
                                    <v-tooltip bottom color="primary" v-if="requestType == 'Claims'">
                                        ...
                                        <span>
                                            <p style="color: white !important;"
                                                class="mb-0 font-weight-light grey-heading-text caption">Here, you can raise
                                                a claim request for the expenses you made. Make sure that all the below
                                                points are followed. </p>
                                            <ul class="pt-0 font-weight-light white-text caption">
                                                <li class="">All fields are mandatory.</li>
                                                <li class="">If you don't find your claim type, please select "Others"
                                                    category and mention the details in the description.</li>
                                                <li class="">Attaching a receipt is compulsory. Make sure the file size is
                                                    not more than 2MB.</li>
                                            </ul>
                                        </span>
                                    </v-tooltip>
                                    <v-img
                        ...
                        <v-col cols="12" sm="12" md="12" class="py-0 mb-5 scroll" style="max-height:600px" align="left"
                            ...
                            <CreateLeaves v-if="fetchedEmployee.length > 0" :selectedEmp="fetchedEmployee[0]"
                                :userType="'ADMIN'" :user="user" :userCompany="getCompanyInformation"
                                :configuration="configData" :users="users" :leave='leave' />
                        </v-col>
                        <v-col cols="12" sm="12" md="12" class="py-0 mb-5 scroll" style="max-height:600px" align="left"
                            ...
                            <v-select @change="fetchUserInfo()" :items="getEmployee"
                                ...
                                hint="Employee" persistent-hint placeholder="Employee" v-model="selectedEmployeeId" dense
                                class="customMdiMenuDown" style="max-width: 73%;margin: 0 auto;"></v-select>
                            <CreateWfh v-if="getSelectedEmployee.length > 0" :selectedEmp="getSelectedEmployee[0]"
                                :request="request_wfh" :employees="[]" :user="user" :configuration="configData"
                                :users="users" :userType="'ADMIN'" />
                        </v-col>
                        <v-col cols="12" sm="12" md="12" class="py-0 mb-5 scroll" style="max-height:600px" align="left"
                            v-if="requestType == 'Passport Safekeep'">
                            <v-select @change="fetchUserInfo()"
                                :items="getEmployee.filter((a) => (a.user_status != 'Inactive' && a.documents.passport_safekeep_required == true && a.documents.passport_submitted == false))"
                                :item-text="(item) => `${item.first_name} ${item.last_name}`" item-value="_id"
                                hint="Employee" persistent-hint placeholder="Employee" v-model="selectedEmployeeId" dense
                                class="customMdiMenuDown" style="max-width: 73%;margin: 0 auto;"></v-select>
                            <CreatePassportSafekeep v-if="getSelectedEmployee.length > 0"
                                :selectedEmp="getSelectedEmployee[0]" :request="request_passport_safekeep" :employees="[]"
                                :user="user" :configuration="configData" :users="users" :userType="'ADMIN'" />
                        </v-col>
                        <v-col cols="12" sm="12" md="12" class="py-0 mb-5 scroll" style="max-height:600px" align="left"
                            v-if="requestType == 'Loan'">
                            <v-select @change="fetchUserInfo()"
                                :items="getEmployee.filter((a) => a.user_status != 'Inactive')"
                                :item-text="(item) => `${item.first_name} ${item.last_name}`" item-value="_id"
                                hint="Employee" persistent-hint placeholder="Employee" v-model="selectedEmployeeId" dense
                                class="customMdiMenuDown" style="max-width: 73%;margin: 0 auto;"></v-select>
                            <CreateLoan v-if="getSelectedEmployee.length > 0" :selectedEmp="getSelectedEmployee[0]"
                                :request="request_loan" :employees="[]" :user="user" :configuration="configData"
                                :users="users" :userType="'ADMIN'" />
                        </v-col>
                        <v-col cols="12" sm="12" md="12" class="py-0 mb-5 scroll" style="max-height:600px" align="left"
                            ...
                            <v-select @change="fetchUserInfo()" :items="getEmployee"
                                ...
                                hint="Employee" persistent-hint placeholder="Employee" v-model="selectedEmployeeId" dense
                                class="customMdiMenuDown" style="max-width: 73%;margin: 0 auto;"></v-select>
                            <ClaimRequest v-if="getSelectedEmployee.length > 0" :claimRequest="false" :employees="[]"
                                :userType="'ADMIN'" :request='request_claims' :user='user'
                                :selectedEmp="getSelectedEmployee[0]" :companyData='companyData' />
                        </v-col>
                        <v-col cols="12" sm="12" md="12" class="py-0 mb-5 scroll" style="max-height:600px" align="left"
                            ...
                            <EditAttendance
                                ...
                                :data='selectedAttendance[0]' :companyData='companyData' :users='users' :userType='"ADMIN"'
                                :selectedEmp="getSelectedEmployee[0]" :user='user'
                                @on-submitted="resetLeaveFormFn(); arr_letters = []; skipCount = 0; getAllRequestsCount(), fetchRequestsUsers()" />
                        </v-col>
                        <v-col cols="12" sm="12" md="12" class="py-0 mb-5 scroll" style="max-height:600px" align="left"
                        ...
                        <v-row class="pt-0 pl-0 mr-0 ml-0" style="align-items: center;">
                            ...
                            <v-col cols="auto" class="py-0 ">
                                <v-text-field v-model="searchRequests" @input="searchFn()" solo flat dense hide-details
                                    prepend-inner-icon="mdi-magnify" class="rounded-xl" placeholder="Search User"
                            ...
                            <v-col cols="auto" class="pa-0" align-self="end">
                                <v-img src="/directory/add_plus.svg" max-width="35" height="auto"
                                    @click="createRequest = true, leavePlanner = true, requestInfo = false, showHeaderPlanner = false"
                        ...
                        <div v-else style="max-width:100%;min-height:100%;">
                            <v-row class="mx-0" style="max-width:100%;min-height:620px">
                                ...
                                <v-col cols="12" v-else justify="center" align="center" class="ma-auto">
                                    ...
                                    <p class="mb-0 grey-heading-text textFontSize mt-5">You did not apply any request yet!
                                    </p>
                                </v-col>
                            </v-row>
                        </div>
            ...
            <v-col cols="12" md="6" class="pa-0 pr-3">
                <v-card class="rounded-xl " style="box-shadow: 0px  24px 30px #959EA51A;" min-height="720px"
                    ...
                    <v-row class="mx-0" style="max-width:100%" justify="center">
                        <v-col cols="12" class="pa-0 ">
                            <v-card-title>
                                <span class="darkBlue-heading-text subHeadingFontSize">Letter Preview</span>
                                <v-spacer></v-spacer>
                            </v-card-title>
                        </v-col>
                    </v-row>
                                ...
                                <v-row class="mx-0" style="max-width:100%;min-height:620px">
                                    <v-col cols="12" class="px-5" v-if="getSelectedEmployee.length > 0">
                                            ...
                                            <v-col cols="auto" :hint="replaceLetterkey">
                                                ...
                                                <div style="justify-content: center;align-items: center;display: flex;">
                                                    <p style="align-items:center;justify-content:center;"
                                                        class="mb-0 caption grey--text">Generating Preview</p>
                                                </div>
                                            </v-col>
                                        </v-row>
                                        <v-row v-else
                                            style="min-height:100%; min-width: 100%; max-width: 100%;align-items:center;justify-content:center;">
                                        ...
                                        <LetterPreview v-if="isLetterRequest(format)"
                                            ...
                                            :signature="format.company_signatory ? getCompanyInformation.letterDetail.signatureLink : getUserCompanyManager(format).signature"
                ...
                <v-card class="rounded-xl scroll" style="box-shadow: 0px  24px 30px #959EA51A;overflow-x:hidden;"
                    ...
                    <v-card-title class="px-1 px-6" v-if="showHeaderPlanner == true">
                        <v-img src="/hr/leavePlanner.svg" max-width="fit-content" height="fit-content" class="mr-2"
                            contain></v-img>
                        <span class="darkBlue-heading-text subHeadingFontSize">Team Calendar</span>
                        <v-spacer></v-spacer>
                    ...
                    <v-sheet height="64">
                        <v-toolbar flat>
                            ...
                            <v-spacer></v-spacer>
                            <v-btn @click="type = 'month'" class="blue-grey--text" outlined dense small
                                v-if="type == 'day'">Month</v-btn>
                    ...
                    <v-sheet class="rounded-lg scroll" height="650" max-height="590">
                        <v-calendar ref="calendar" v-model="focus" color="primary" :events="events"
                            ...
                            @click:date="viewDay" @change="updateRange" title-position="right"></v-calendar>
                ...
                <v-card class="rounded-xl overflow-hidden" style="box-shadow: 0px  24px 30px #959EA51A;" min-height="720px"
                    ...
                    <v-sheet class="rounded-lg scroll" height="650" max-height="590">
                        <v-calendar ref="calendar_wfh" v-model="focus_wfh" color="primary" :events="events_wfh"
                            :event-color="getEventColor" :type="type_wfh" @click:event="showEvent_wfh"
                ...
                <v-card class="rounded-xl " style="box-shadow: 0px  24px 30px #959EA51A;" min-height="720px"
                    ...
                    <v-row class="mx-0" style="max-width:100%" justify="center">
                            ...
                            <v-card-title>
                                <v-col cols="4" class="pa-0">
                                    <v-btn outlined min-width="90" color="#5C7EEF"
                                        ...
                                        @click="loadHistory()">{{ viewRequestHistory == true ? 'View Pending' :
                                            'ViewHistory' }}</v-btn>
                                </v-col>
                                <v-spacer></v-spacer>
                                <v-col cols="5" class="pa-0">
                                    <v-select id="custom" class="rounded-xl w-50 customMdiMenuDown customA"
                                        :items="taskFilters" outlined v-model="selectedFilterDate">
                        ...
                        <v-col cols="10" class="pt-0">
                            <v-row class="mx-0" style="max-width:100%">
                                <v-col sm="5" md="5" lg="6" xl="7">
                                    <p class="font-weight-medium subHeadingFontSize mb-0"
                                        v-if="viewRequestHistory == false">Recent Request</p>
                                    ...
                                    <p class="font-weight-normal fontSize1" v-if="viewRequestHistory == false"><span
                                            class="customRed fontWeight600">{{ totalCount }}</span> requests are pending for
                                        approvals</p>
                                    <p class="font-weight-normal fontSize1" v-else><span class="customRed fontWeight600">{{
                                        totalCount }}</span> total requests </p>
                                </v-col>
                                    ...
                                    <span>
                                        ...
                                        <v-menu nudge-left="100" v-if="requests_users.length > 0">
                                            ...
                                            <v-list max-height="281px" class="scroll">
                                                <v-text-field v-model="filterName" label="Filter by name" @click.stop
                                                    autofocus
                                                    ...
                                                    <v-list-item-action class="ma-0">
                                                        <v-list-item-action-text class="caption mt-2 ml-4">
                                                            <v-list-item-action-text
                                                                ...
                                                                style="min-width: 25px;justify-content: flex-end;">
                                                ...
                                                <v-list-item v-if="filteredRequestsUsers.length === 0">
                                                    <v-list-item-content>
                                                        <v-list-item-title>No user found</v-list-item-title>
                                                    </v-list-item-content>
                                                </v-list-item>
                                            </v-list>
                                        </v-menu>
                        ...
                        <v-col cols="10" class="pt-2">
                            <RequestsFiltering :adminCentral='true' :loading='loading' :configData='configData'
                ...
                <v-card class="rounded-xl " style="box-shadow: 0px  24px 30px #959EA51A;" min-height="720px" v-else>
                    <v-card-title class="px-1 px-6 py-5">
                        ...
                        <h3 class="darkBlue-heading-text subHeadingFontSize">Request Information {{ requestType }}</h3>
                        <v-spacer></v-spacer>
                        <template v-if="isDraftClaimRequest">
                            <v-btn color="error" class="mr-2" @click.prevent="deleteDraftClaimReq">
                                DELETE
                            </v-btn>
                            ...
                            <v-row class="mx-0" style="max-width:100%;min-height:620px">
                                <v-col cols="12" class="px-5">
                                    <TeamRequestInfo :users="users" :countryList="countryList" :configData='configData'
                                        :user="user" :adminCentral='true' :data="computedServiceList"
                                        :companyData='companyData' :emailBody="emailBody"
                                        :selectedEmp="getSelectedEmployee[0]" :teamCentral='true' :userType="'ADMIN'" />
                                </v-col>
                            </v-row>
                        </div>
                    </div>
            ...
            <v-card class="rounded-xl pa-0 pt-0" flat min-height="230">
                    ...
                    <v-col cols="6" sm="6" class="pb-2">
                        <v-card-title class="py-0">
                            <v-img src="/shift/build.svg" max-width="fit-content" height="fit-content" class="mr-2"
                                contain></v-img>
                            <span class="darkBlue-heading-text font-weight-normal subHeadingFontSize"> Select Custom
                                Date</span>
                        </v-card-title>
                    </v-col>
                </v-row>
                ...
                <v-row justify="center" class="px-5 mt-7">
                    <v-col cols="12" class="pb-2 ">
                        <v-card-title class="py-0">
                            <span class="darkBlue-heading-text font-weight-normal subHeadingFontSize mr-5">Date Range
                            </span>
                            <v-menu ref="menu_calendar_button" v-model="menu_calendar_button"
                                ...
                                <v-date-picker range v-model="date_calendar" no-title scrollable>
                                    ...
                                    <v-btn text color="primary"
                                        @click="$refs.menu_calendar_button.save(date_calendar); customdate(date_calendar);">OK</v-btn>
                                </v-date-picker>
                            </v-menu>
                        </v-card-title>
                    ...
                    <!-- <v-col cols="11"  class="pb-2" style="text-align: end;">
                        <download-excel :name="date_calendar.length == 1 ? selectedFilter + ' Requests (' + date_calendar[0] +')' : selectedFilter + ' Requests (' + date_calendar[0] + ' - ' +  date_calendar[1] +')'" v-if="dateRangeText != ''" :data="getInfoForDownload" :fields='getFields()'>
                    ...
                    <v-col cols="12" sm="12" class="pb-2">
                        <v-card-title class="py-0">
                            ...
                            <span class="darkBlue-heading-text font-weight-normal subHeadingFontSize">Download Report -
                                {{ capitalizeFirstLetter(requestFilterDownload(selectedFilter)) }}</span>
                        </v-card-title>
                    </v-col>
                </v-row>
                <v-divider></v-divider>
                        ...
                        <v-card-title class="py-0">
                            ...
                            <v-menu ref="menu_calendar_button2" v-model="menu_calendar_button2"
                                ...
                                <v-date-picker range v-model="date_calendar_download" no-title scrollable
                                    @input="fetchDataForExcelDownload()" :min="date_calendar_download[0]">
                                    <v-btn text color="red" @click="resetCalendarFilterDateDownload">Clear</v-btn>
                                    <v-spacer></v-spacer>
                                    <v-btn text color="grey" @click="menu_calendar_button2 = false">Cancel</v-btn>
                                    <v-btn text color="primary"
                                        @click="$refs.menu_calendar_button2.save(date_calendar_download)">OK</v-btn>
                                </v-date-picker>
                            </v-menu>
                        </v-card-title>
...
import CreatePassportSafekeep from "~/components/reuseable/createPassportSafekeep.vue";
...
export default {
    components: {
        RequestListing, LetterDownload, TeamRequestInfo, Observer, CreateLeaves, CreateWfh, ClaimRequest, CreateLoan, RequestsFiltering, CreatePassportSafekeep,
    ...
    props: ['configData', 'companyData', 'users', 'user', 'roles'],
    data() {
        return {
            filterName: '',
            compensationTable: "",
            ...
            requestType: '',
            requestTypeList: ['Leave', 'Letters', 'Claims', 'wfh'],
            leave: {
                appliction_log: [],
                leave_condition: {},
                leave_type: "",
            ...
            requestFilterType: ['ALL', 'Leave', 'letters', 'wfh', 'attendance', 'claims'],
            downloadReportType: ['Leave', 'letters', 'wfh', 'attendance', 'claims'],
            selectedItem: 0,
            emailBodyHR: {
                hr_email: "",
                email: "",
            ...
            request_passport_safekeep: {
                request_type: 'passport safekeep',
                status: '',
                approvals: [],
                letter_fields: {
                    date: new Date().toISOString().substr(0, 10),
    ...
    created() {
        ...
        this.$nuxt.$on('addNewSalaryAdjustment', async ($event) => {
    ...
    beforeDestroy() {
        this.$nuxt.$off('user_changed', 'letterUrl', 'downloadLetterApprove', 'addNewClaim', 'resetLeaveForm', 'userSelected_letter', 'refreshRequests')
        this.breakloop = true;
    ...
    methods: {
        ...
        getRoleName(val) {
            let roles = this.roles.filter((a) => a._id == val);
            return roles.length > 0 ? roles[0].role_name : "";
        },
        replaceCustomKeys(userLetterInfo, letterInfo, bln_newLetter) {
            let result = letterInfo
            let letterKeys = this.filterLetterFormats.letterKeys
            /**
            ...
            */
            ...
            if (bln_newLetter) {
                ...
                // Loan Amount
                var t2w_loanAmount = titleCase(titleCaseDash(translator.toWords(parseInt(loanAmount))));
                var t2wFils_loanAmount = titleCase(titleCaseDash(translator.toWords(parseInt((parseFloat(loanAmount).toFixed(2) + "").split(".")[1]))));
            ...
            for (let index = 0; index < letterKeys.length; index++) {
                if (letterKeys[index].name.key == "[newEmpBasicSalary]") {
                    result = result.replaceAll('[newEmpBasicSalary]', parseFloat(letterKeys[index].value) ? parseFloat(letterKeys[index].value).toLocaleString("en-US") : 0)
                } else if (letterKeys[index].name.key == "[currentAllowance]") {
                    result = result.replaceAll('[currentAllowance]', parseFloat(letterKeys[index].value) ? parseFloat(letterKeys[index].value).toLocaleString("en-US") : 0)
                } else if (letterKeys[index].name.key == "[newAllowance]") {
                    result = result.replaceAll('[newAllowance]', parseFloat(letterKeys[index].value) ? parseFloat(letterKeys[index].value).toLocaleString("en-US") : 0)
                }
            }
             ...
             * Emirates Expiry ID
             */
            let eid_expiry = bln_newLetter ?
                userLetterInfo.documents.hasOwnProperty('emiratesID_expiry') ? moment(userLetterInfo.documents.emiratesID_expiry).format('D MMMM YYYY') : '' :
                userLetterInfo.hasOwnProperty('emiratesID_expiry') ? userLetterInfo.emiratesID_expiry : ''

            result = result.replaceAll('[emiratesExpiry]', eid_expiry)

            /**
             * Date Today
             */
            let date_today = bln_newLetter ?
                moment(new Date()).format('D MMMM YYYY') :
                userLetterInfo.hasOwnProperty('date_today') ? userLetterInfo.date_today : ''

            result = result.replaceAll('[dateToday]', date_today)
            ...
            let manager_role = ''
            if (bln_newLetter) {
                let manager_info = this.getUsersInfoForExcel(userLetterInfo.reporting.manager);
                manager = manager_info ? manager_info.first_name + " " + manager_info.last_name : ''
                manager_role = manager_info ? this.getRoleName(manager_info.role_ID) : ''
            } else {
                manager = userLetterInfo.hasOwnProperty('manager_name') ? userLetterInfo.manager_name : ''
                manager_role = userLetterInfo.hasOwnProperty('manager_role') ? userLetterInfo.manager_role : ''
            }
            result = result.replaceAll('[lineManager]', manager)
            result = result.replaceAll('[lineManagerRole]', manager_role)
        ...
        async getNewLetterPreview() {
            ...
            let funGetUserInfo = function (bln_newLetter, obj_userInfo, obj_requestInfo) {
                ...
                obj_user.basic_salary = bln_newLetter ? obj_userInfo.salary.basic_salary : obj_requestInfo.letter_fields.basic_salary;
            ...
            sampleRequest.letter_fields['fixedOvertime'] = user_data.fixed_overtime
        ...
        requestFilterDownload(filterName) {
            ...
            if (filterName == 'salary') return 'Salary'
        },
            ...
            try {
                await this.$axios.$post('requests/get_all_requests_count/' + this.user._id, body)
                    .then(res => {
                        ...
                        this.salaryCount = res.data.salaryAdjustmentCount ? res.data.salaryAdjustmentCount : 0;
                ...
                // await this.$axios.$post('salaryAdjustment/get_all_requests_count', { "history": this.viewRequestHistory })
                //     .then(res => {
        ...
        async getAllSalaryRequest() {
            await this.$axios.$post('salaryAdjustment/get_all_requests', { "history": this.viewRequestHistory })
                .then(res => {
        ...
        requestListScrollerObserver() {
            if (this.selectedFilter == "salary") {

            }
            else if (this.totalCount > this.arr_letters.length) {
                this.getAllRequests()
            }
        },
        ...
        async getTeamWfhList() {
            ...
            if (this.getSelectedEmployee.length > 0) {
                ...
                await this.$axios.$post("/wfh/dept", body, { headers: { Authorization: AuthStr } })
                    .then((res) => {
        ...
        computeRequestTypeList() {
            let requestTypeList = this.configData[0].requestTypes
            let arr = []
            for (let index = 0; index < requestTypeList.length; index++) {
                ...
                if (element == 'Passport Release/Safekeep') {
                    obj.name = 'Passport Safekeep'
                    obj.value = 'Passport Safekeep'
                    arr.push(obj)
                } else {
                    arr.push(obj)
    ...
    computed: {
        ...
        getInfoForDownload() {
            ...
            if (this.dateRangeTextExcel != '') {
                if (this.excel_downloaded_data == '' || this.excel_downloaded_data == undefined || this.excel_downloaded_data == null) {
                    ...
                    if (this.selectedFilter == 'letters' || this.selectedFilter == 'claims' || this.selectedFilter == 'attendance' || this.selectedFilter == 'loan' || this.selectedFilter == 'education' || this.selectedFilter == 'passport') {
                        // if(this.date_calendar_download.length == 1){
                        ...
                        // if(this.date_calendar_download.length == 1){
                        //     abc = clonedData.filter(a=>{
                        //         let CDate = moment(String(a.from_date)).format('DD MMM, YYYY')
                        //         let FDate = moment(String(this.date_calendar_download[0])).format('DD MMM, YYYY')
                        //         return CDate == FDate
                        //     })
                        // }
                        // else  if(this.date_calendar_download.length > 1){
                        //     abc = clonedData.filter(a=>{
                        //         let CDate = new Date(a.from_date)
                        //         return CDate >= fromDate && CDate <= toDate
                        //     })
                        // }
                    ...
                    if (this.selectedFilter == 'letters') {
                        ...
                        for (let index = 0; index < abc.length; index++) {
                            ...
                            // allObj.from_date=abc[index].from_date 
                            ...
                            // allObj.description=abc[index].hasOwnProperty('letter_fields') ? abc[index].letter_fields.description : abc[index].letter_fields.comments
                            ...
                            // allObj.requestedDate= abc[index].letter_fields.date ? this.formatDate(abc[index].letter_fields.date) : ""
                            // allObj.approvedDate=this.formatDate(abc[index].approvals.length >0 ? abc[index].approvals[abc[index].approvals.length-1].approved_date  : "") || ""
        ...
        replaceLetterkey() {
            ...
            this.request.user_keys = this.filterLetterFormats.user_keys
            let userInfo = this.getSelectedEmployee[0]
            // if(userInfo.personal.gender == 'Male') {
            // }else {
            // }
        ...
        getSelectedEmployee() {
            let abc = _.cloneDeep(this.users)
            return abc.filter((a) => a._id == this.selectedEmployeeId)
        },
        ...
        getEmployee() {
            let abc = []
            abc = _.cloneDeep(this.users.filter(a => a.user_status == 'Active' || a.user_status == 'Hold' || a.user_status == 'active'))
        ...
        visibleData() {
            ...
            //             let c = _.filter(value, function (val) {
            // 				return getEmpId(val).toLowerCase().indexOf(s.toLowerCase()) > -1;
            // 			});

            // 			let resultempid = _.filter(c, function (val) {
            // 				return getEmpId(val) != null;
            // 			});
            // 			return (
        ...
        computedRequestUsers() {
            // let arr = []
            // let value = _.sortBy(this.filterValue,['user_data.first_name','user_data.last_name'])
