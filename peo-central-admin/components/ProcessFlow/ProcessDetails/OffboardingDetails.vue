<template>
  <v-row>
    ...
    <v-col cols="12" v-else>
      <v-card class="no-border_shadow" color="card_bg" id="card" max-height="555px">
              ...
              <v-data-table id="coa_table" class="main__table elevation-0 customDataTabel  th_customer mb-5"
                ...
                <template v-slot:item="{ item, }">
                  ...
                  <tr>
                    ...
                    <td class="pb-2 pt-2">{{ item.phone ? item.phone : '-' }} </td>
                    <td class=""> </td>
                  </tr>
                </template>
              </v-data-table>
...
export default {
  ...
  methods: {
    async getEmployeeDetails(id) {
      ...
      await this.$axios.$post(`/offboardings/employee/details/${this.process_id}`, { headers: { Authorization: AuthStr } })
        .then((response) => {
          this.loading = false
        ...
        .then(async (response) => {
          ...
          if (this.selectedEmployeeDetails && this.selectedEmployeeDetails.length > 0 && this.selectedEmployeeDetails[0].hasOwnProperty('employment') && this.selectedEmployeeDetails[0].employment.visa_sponsor_type != '') {
            if (this.selectedEmployeeDetails[0].employment.visa_sponsor_type == 'Dynamic Employment Services') {
              ...
              this.$emit('onCompanyIds', { parent_company : this.parent_company, parent_company_id: this.parent_company_id })
            }
