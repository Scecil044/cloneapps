<template>
  <v-row class="onboarding-visa pb-4">
    ...
    >
      <Steps
        @updated="getVisaProcessList($event)"
    ...
    >
      <VisaProcessList
        ...
        :sub-stage-name="subStageName"
        :new-pro-list="newProList"
...
export default {
  ...
  mounted() {
    this.subStageName = "mol offer letter";
    this.generateVisaProcessList();
  },
  methods: {
    updateProcess(event) {
    ...
    async generateVisaProcessList($event) {
      ...
      try {
        ...
        console.log("The onboarding generated visa process list is", response.results)
        this.ShowList = false;
        this.newProList = response.results;
        this.ShowList = true;
      } catch (error) {
