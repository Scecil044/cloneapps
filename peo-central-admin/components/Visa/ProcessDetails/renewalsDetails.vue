<template>
  <v-row>
    <v-col cols="12" v-if="loading && employeeDetails.length <= 0">
      <v-card class="d-flex align-center" color="card_bg" id="card" style="max-height: 58vh">
        <v-progress-linear indeterminate color="primary"></v-progress-linear>
      </v-card>
    </v-col>
    <v-col cols="12" v-else>
      <v-card class="no-border_shadow" color="card_bg" id="card" style="max-height: 85vh">
            ...
            <template v-slot:item="{ item, index }">
              <tr>
                ...
                <td class=" d-flex align-center">
                  <img :src="item.company_logo" style="border-radius: 10px" width="30" height="30" v-if="index === 0"
                    class="mr-2" />
...
export default {
  ...
  methods: {
    async getEmployeeDetails(id) {
      ...
      await this.$axios.$post(`/renewals/employeeDetails/${this.process_id}`, { headers: { Authorization: AuthStr } })
        .then((response) => {
          this.loading = false
