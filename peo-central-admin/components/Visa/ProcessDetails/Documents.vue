<template>
    ...
    ></v-progress-linear>
          ...
          <v-data-table id="coa_table" :search="search" class="main__table elevation-0 document_table"
            ...
            <template v-slot:item="{ item, index }">
              <tr class="table_row">
                ...
                <td class="" @click="openDocument(item.url)">{{ item.type_name }}</td>
                ...
                <td class="" @click="openDocument(item.url)">{{ item.expiry | formatDateWithoutTime }}</td>
                <td class="pa-0 ma-0 pl-3">
                  <div class="">
                    <span class="table_btn light_accent4 accent4--text" v-if="item.doc_status == 'valid'">{{ item.doc_status
                    }}</span>
                    <span class="table_btn light_accent3 accent3--text" v-if="item.doc_status == 'Soon Expiring'">{{ item.doc_status
                    }}</span>
                    <span class="table_btn light_accent2 accent2--text"
                      v-if="item.doc_status == 'Expired' || item.doc_status == 'expired'">{{ item.doc_status }}</span>
                  </div>
                </td>
                <td class="">
                  <div class="d-flex align-center justify-space-between">
                    ...
                    <v-tooltip top color="grey">
                          <template v-slot:activator="{ on, attrs }">
                            <EditSvg v-bind="attrs" v-on="on" @click="editDocument(item)" />
                          </template>
                          Edit
                      </v-tooltip>
                    <!--</span>
                    ...
                    <!-- <span class="">
                      <v-tooltip top color="grey">
                        <template v-slot:activator="{ on, attrs }">
                          <DownloadSvg v-bind="attrs" v-on="on" @click="downloadFile(item.url)" />
                        </template>
                        Download
                      </v-tooltip>
                    </span> -->
      ...
      <DocumentAddition @close="documentAdd = false" :type="'new'" @updated="updatedDocument()" v-if="documentAdd"
        :module="'users'" :foreign_id="this.employeeDetails[0]._id" />
  
      <!--Document Edit-->
      <DocumentAddition :document="selectedDocument" :type="'edit'" @close="documentEdit = false"
      :module="'users'"   @updated="updatedDocumentEdit()" v-if="documentEdit" />
