    ...
    <v-card-text id="card-text">
      ...
      <div class="report_body flex_column align-center justify-center">
        <div class="re_title my-5 flex_column align-center justify-center">
          <h4 class="font-wight-bold text--text mb-2" >Profit and Loss</h4>
        </div>
          <!-- report-body -->
          <div class="report-body" v-if="dataLoaded">
                    ...
                    <v-expansion-panels>
                      ...
                      >
                        ...
                        <v-expansion-panel-content>
                          <v-row v-for="(data,index) in item.list" :key="index" style="min-height:max-content !important"  @click="openPNLItem(data)">
                            <v-col class="pa-0 text-left" cols="6" >
                              <p>{{ data.name }}</p>
                            </v-col>
                            <v-col class="pa-0 text-right" cols="6" >
                              <p>{{ data.amount | twoDecimals}}</p>
                            </v-col>
                          </v-row>
                        </v-expansion-panel-content>
                        <v-divider></v-divider>
                        <v-row style="min-height:max-content !important" class="px-4 pt-3">
                          ...
                          <v-row style="min-height:max-content !important" class="px-4 pt-0" v-if="i == 1">
                            <v-col class="pa-0 text-left " cols="6" >
                              <p class="font-weight-normal">GROSS PROFIT</p>
                            </v-col>
                            <v-col class="pa-0 text-right" cols="6" >
                              <p class="font-weight-medium">AED {{ data_item[0].subtotal - data_item[1].subtotal  | twoDecimals}}</p>
                            </v-col>
                          </v-row>
                      </v-expansion-panel>
              ...
              <tfoot>
                  <tr>
                  <th class="text-left">NET EARNINGS</th>
                  <th class="text-right">AED {{computedNetEarning | twoDecimals}}</th>
                  </tr>
              </tfoot>
              </table>
...
export default {
  ...
  methods:{
    ...
    async getReport(){
      ...
      await this.$axios
        .$post(`reports/profit-loss`, payload, {
          headers: { Authorization: AuthStr },
        })
        .then(async (res) => {
          for ( let i = 0 ; i < res.subtotal.length ; i++){
            res.subtotal[i].list = (res.subtotal[i].list.filter(j => j.JE.length > 0))
            if(res.subtotal[i].name == 'Income'){
              res.subtotal[i].priority = 1
            }
            if(res.subtotal[i].name == 'Cost of Sales'){
              res.subtotal[i].priority = 2
            }
          }
          console.log(res.subtotal)
          let arr = _.sortBy(res.subtotal,'priority')
          this.data_item = arr
  ...
  computed:{
    ...
    computedNetEarning(){
      if(this.data_item && this.data_item.length > 1){
        return ((this.data_item[0].subtotal - this.data_item[1].subtotal)  - this.data_item[2].subtotal)
      }
    }
  }
