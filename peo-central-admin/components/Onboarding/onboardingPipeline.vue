<template>
  <v-row class=" d-flex flex-nowrap pipeline_cards" v-if="!loading">
    <v-col cols="3" v-for="(item, index)  in onboardingPipeline" :key="index">
      <pipelineCard :leadsData="item" :index="index" module="onboardings" />
    </v-col>
  </v-row>
</template>
<script>
...
export default {
  ...
  methods: {
    async getPipelineCount() {
      ...
      await this.$axios.$post('/onboardings/pipeline', { headers: { Authorization: AuthStr } })
        .then(async (response) => {
          this.onboardingPipeline = response
          this.loading = false
        })
    }
  }
