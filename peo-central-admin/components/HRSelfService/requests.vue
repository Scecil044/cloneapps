  ...
  <div class="pt-3" style="width: 100% !important">
    <v-row class="pt-0 ml-0 pr-5">
          ...
          <v-row class="mx-0" style="max-width: 100%" justify="center">
            <v-col cols="12" class="pa-0">
              <v-card-title>
                <v-select
                  ...
                  dense
                  outlined
                  class="customMdiMenuDown rounded-xl textFieldDetailsCuttom"
                  style="max-width: 43%; margin: 0 auto"
                ></v-select>
                <v-row style="float: right">
                  ...
                  >
                    <template v-slot:activator="{ on, attrs }">
                      <v-img
                        @click="newClaim = false"
                        src="/hr/information.png"
                        class="mx-2"
                    ...
                    <span>
                      ...
                      >
                        Here, you can raise a claim request for the expenses you
                        made. Make sure that all the below points are followed.
                      </p>
                      <ul class="pt-0 font-weight-light white-text caption">
                        <li class="">All fields are mandatory.</li>
                        <li class="">
                          If you don't find your claim type, please select
                          "Others" category and mention the details in the
                          description.
                        </li>
                        <li class="">
                          Attaching a receipt is compulsory. Make sure the file
                          size is not more than 2MB.
                        </li>
                      </ul>
                    </span>
                  </v-tooltip>
            ...
            >
              <CreateLeaves
                ...
                :userType="'SELF'"
                :user="user"
                :userCompany="getCompanyInformation"
            ...
            <v-col
              ...
              style="max-height: 600px"
              align="left"
              v-if="requestType == 'Education'"
            >
              <CreateEducation
                :employees="[]"
                :userType="'SELF'"
                :request="request_education"
                :user="user"
                :selectedEmp="user"
                :companyData="companyData"
              />
            ...
            >
              <LetterRequest
                v-if="configData[0].letterRequest.length > 0"
                :countryList="countryList"
                :NOCOptions="NOCOptions"
                ...
                :request="request"
                :getUserDetails="[user]"
                :emailBodyHR="emailBodyHR"
                :emailBody="emailBody"
                :user="user"
                :addressee="
                  (this.filterLetterFormats.letterDescription.header = !''
                    ? this.filterLetterFormats.letterDescription.header
                    : replaceLetterkey)
                "
                :subject="computeNewLetterSubject"
                :body="
                  (this.filterLetterFormats.letterDescription.body = !''
                    ? this.filterLetterFormats.letterDescription.body
                    : replaceLetterkey)
                "
                ...
                :letterKeys="
                  this.filterLetterFormats.letterKeys
                    ? this.filterLetterFormats.letterKeys
                    : []
                "
                :companyData="companyData"
                :totalFixedWord="totalFixedWord"
                :basicSalaryInWords="basicSalaryInWords"
                :loanAmountInWords="loanAmountInWords"
                :compensationTable="compensationTable"
                :personalLoanTable="personalLoanTable"
                :personalLoanTable2="personalLoanTable2"
                :userType="'SELF'"
                :personalLoanTable3="personalLoanTable3"
              />
              <div v-else>
                <p>No letter type configured</p>
              </div>
            </v-col>
            ...
            <v-col
              ...
              style="max-height: 600px"
              align="left"
              v-else-if="requestType == 'Passport Release'"
            >
              <CreatePassportRelease
                :employees="[]"
                :userType="'SELF'"
                :request="request_passport_release"
                :user="user"
                :selectedEmp="user"
                :companyData="companyData"
              />
              ...
              <v-row class="mx-0" style="max-width: 100%; min-height: 620px">
                ...
                >
                  ...
                  <p class="mb-0 grey-heading-text textFontSize mt-5">
                    You did not apply any request yet!
                  </p>
                </v-col>
              </v-row>
            </div>
      ...
      <v-col cols="12" md="6" class="pa-0 pr-3">
          ...
          <div>
            <div class="scroll" style="max-height: 620px; min-height: 620px">
              <div style="max-width: 100%; min-height: 100%">
                <v-row class="mx-0" style="max-width: 100%; min-height: 620px">
                  <v-col cols="12" class="px-5" v-if="user">
                    <!-- {{ letterLoadingNew }}
                                        {{ filterLetterFormats }} -->
                    <v-row
                      v-if="letterLoadingNew && filterLetterFormats"
                      style="
                        min-height: 100%;
                        align-items: center;
                    ...
                    <!-- <div v-for="(format, index2) in configData[0].letterRequest" :key="index2">
                                            <LetterPreview v-if="isLetterRequest(format)" :date="computeDateNewRequest"
                                                :dateClass="format.formattedText.formatDate" :toAddress="replaceHeader"
                                                ...
                                                :subjectClass="format.formattedText.formatStyle" :body="replaceLetterkey"
                                                :inlineStyle="inlineStyle" :leftSidebarLink="leftSidebarLink"
                                                :rightSidebarLink="rightSidebarLink" :leftSidebarCol="format.leftSidebarCol"
                                                :bodyCol="format.bodyCol" :rightSidebarCol="format.rightSidebarCol"
                                                ...
                                                :signatory="format.signatory.preview" :header="format.headerShow.preview"
                                                :footer="format.footerShow.preview" :stamp="format.stampShow.preview"
        ...
        <v-card
          class="rounded-xl scroll"
          style="box-shadow: 0px 24px 30px #959ea51a; overflow-x: hidden"
          min-height="720px"
          max-height="720px"
          v-else-if="requestType == 'Leave' && leavePlanner == true"
        >
          <v-card-title class="px-1 px-6" v-if="showHeaderPlanner == true">
            <v-img
              src="/hr/leavePlanner.svg"
              max-width="fit-content"
              height="fit-content"
              class="mr-2"
              contain
            ...
            <v-spacer></v-spacer>
            <v-btn
              class="pa-0 border-radius-medium"
              color="#FF0000"
              outlined
              @click="leavePlanner = false"
              style="
                min-width: 30px;
                max-width: 30px;
                height: 30px;
            ...
            >
              <v-card color="grey lighten-4" min-width="300px" flat>
                <v-toolbar :color="selectedEvent.color" dark>
                  ...
                  ></v-toolbar-title>
                  <span class="pt-1"
                    >&nbsp; - {{ changeName(selectedEvent.status) }}</span
                  >
                </v-toolbar>
                <v-row class="pl-2 pr-2">
                  <v-col>Leave Type :</v-col>
        ...
        <v-card
          ...
          min-height="720px"
          max-height="720px"
          v-else-if="leavePlanner == true && requestType == 'wfh'"
        >
          <v-card-title class="px-1 px-6" v-if="showHeaderPlanner == true">
            <v-img
              src="/hr/leavePlanner.svg"
          ...
          <v-sheet class="rounded-lg scroll" height="650" max-height="590">
            <v-calendar
              ...
              :event-color="getEventColor"
              :type="type_wfh"
              @click:event="showEvent_wfh"
              @click:more="viewDay_wfh"
              @click:date="viewDay_wfh"
              @change="updateRange_wfh"
              title-position="right"
            ></v-calendar>
            <v-menu
              v-model="selectedOpen_wfh"
              :close-on-content-click="false"
              :activator="selectedElement_wfh"
              offset-x
            >
              <v-card color="grey lighten-4" min-width="300px" flat>
                <v-toolbar :color="selectedEvent_wfh.color" dark>
                  <v-toolbar-title
                    v-html="selectedEvent_wfh.name"
                  ></v-toolbar-title>
                  <span class="pt-1"
                    >&nbsp; - {{ changeName(selectedEvent_wfh.status) }}</span
                  >
                </v-toolbar>
                <v-row class="pl-2 pr-2">
          ...
          <v-row class="mx-0" style="max-width: 100%" justify="center">
              ...
              <v-card-title>
                <v-col cols="4" class="pa-0">
                  <v-btn
                    ...
                    min-width="90"
                    color="#5C7EEF"
                    class="textTransformUnset borderRadiusButtons"
                    :disabled="loading"
                    @click="loadHistory()"
                    >{{
                      viewRequestHistory == true
                ...
                <v-col cols="5" class="pa-0">
                  <v-select
                    class="rounded-xl w-50 customMdiMenuDown customA"
                    :items="taskFilters"
                    outlined
            ...
            <v-col cols="10" class="pt-0">
              <v-row class="mx-0" style="max-width: 100%">
                <v-col sm="5" md="5" lg="6" xl="7"> </v-col>
                <v-col sm="7" md="7" lg="6" xl="5"> </v-col>
              </v-row>
            </v-col>
            <v-col cols="10" class="pt-12">
              <RequestsFiltering
                ...
                :computedAllReqCountObj="computedAllReqCountObj"
                :computedTotalReqCount="computedTotalReqCount"
        ...
        >
          <v-card-title class="px-1 px-6 py-5">
            <v-img
              ...
              contain
            ></v-img>
            <h3 class="darkBlue-heading-text subHeadingFontSize">
              Request Information {{ changeRequestTypeText(requestType) }}
            </h3>
            <v-spacer></v-spacer>
            <template v-if="isDraftClaimRequest">
              ...
              <v-btn
                color="primary"
                class="mr-2"
                @click.prevent="openEditClaimReq"
              >
                EDIT
              </v-btn>
            </template>
            ...
            <v-btn
              v-if="requestType == 'Leave' || requestType == 'wfh'"
              outlined
              class="rounded-xl blue-heading-text mr-2"
              color="#5A7DEF"
              @click="leavePlanner = true"
            >
              <v-img
                src="/hr/team.svg"
                max-width="fit-content"
            ...
            <v-img
              ...
              v-if="
                computedServiceList.request_type == 'letters' &&
                (computedServiceList.status == 'Completed' ||
                  computedServiceList.status == 'completed') &&
                letterLoading == false
              "
              contain
      ...
      <v-card class="rounded-xl pa-0 pt-0" flat min-height="230">
        ...
        <v-row justify="center" class="px-5 mt-7">
          <v-col cols="12" class="pb-2">
            <v-card-title class="py-0">
              <span
                class="darkBlue-heading-text font-weight-normal subHeadingFontSize mr-5"
                >Date Range
              </span>
              <v-menu
                ref="menu_calendar_button"
                v-model="menu_calendar_button"
                :close-on-content-click="false"
                :return-value.sync="date_calendar"
              ...
              >
                <template v-slot:activator="{ on, attrs }">
                  <v-text-field
                    ...
                    style="
                      ...
                      box-shadow: 0px 24px 30px #959ea51a;
                ...
                >
                  ...
                  <v-btn
                    ...
                    color="primary"
                    @click="
                      $refs.menu_calendar_button.save(date_calendar)
                      customdate(date_calendar)
                    "
                    >OK
                  </v-btn>
                </v-date-picker>
        ...
        <v-row justify="center" class="px-5 mt-7">
          <v-col cols="12" class="pb-2">
            <v-card-title class="py-0">
              ...
              <v-menu
                ref="menu_calendar_button2"
                v-model="menu_calendar_button2"
                :close-on-content-click="false"
              ...
              >
                ...
                <v-date-picker
                  range
                  v-model="date_calendar_download"
                  no-title
                  scrollable
                ...
                >
                  <v-btn
                    text
                    color="red"
                    @click="resetCalendarFilterDateDownload"
                    >Clear</v-btn
                  >
                  <v-spacer></v-spacer>
                  <v-btn
                    text
                    color="grey"
                    @click="menu_calendar_button2 = false"
                    >Cancel</v-btn
                  >
                  <v-btn
                    text
                    color="primary"
                    @click="
                      $refs.menu_calendar_button2.save(date_calendar_download)
                    "
                    >OK
                  </v-btn>
                </v-date-picker>
          ...
          <v-col cols="12" class="pb-2" style="text-align: end">
            <v-row justify="end">
              <v-btn
                ...
                elevation="0"
                @click="
                  ;(date_calendar_download = []), (downloadReqReport = false)
                "
              ...
              <download-excel
                v-if="dateRangeTextExcel != ''"
                :data="getInfoForDownload"
                :name="
                  selectedFilter == 'ALL'
                    ? `Self Service Requests (${dateRangeTextExcel})`
                    : `Self Service Requests - ${getReqName(
                        selectedFilter
                      )} (${dateRangeTextExcel})`
                "
                :fields="getFields()"
...
import CreateLoan from '~/components/reuseable/createLoan.vue'
import CreateEducation from '~/components/reuseable/createEducation.vue'
...
import CreatePassportRelease from '~/components/reuseable/createPassportRelease.vue'
...
export default {
  components: {
    ...
    PreviewLetter: () => {
      if (process.client) {
        return import('~/components/document-editor/wordsimplepreview.vue')
      }
    },
  },
  props: ['configData', 'companyData', 'users', 'user'],
  data() {
    return {
      compensationTable: '',
      personalLoanTable: '',
      ...
      requestFilterType: ['ALL', 'Leave', 'letters', 'wfh', 'claims'],
      downloadReportType: ['Leave', 'letters', 'wfh', 'claims'],
      selectedItem: 0,
      emailBodyHR: {
        hr_email: '',
        email: '',
      ...
      request_passport_release: {
        request_type: 'passport release',
        status: '',
        approvals: [],
        letter_fields: {
          date: new Date().toISOString().substr(0, 10),
  ...
  created() {
    this.$nuxt.$on('userSelected_letter', ($event) => {
      this.selectedEmployeeId = this.user._id
    }),
      this.$nuxt.$on('resetLeaveForm_admin', async ($event) => {
    ...
    this.$nuxt.$on('letterUrl', async ($event) => {
      // this.getAllRequests(false)
      this.arr_letters = []
      this.skipCount = 0
      this.getAllRequestsCount()
      this.downloadLetters = false
    })

    this.$nuxt.$on('downloadLetterApprove', async ($event) => {
      this.downloadLetters = true
    })
    this.$nuxt.$on('requestWithdrawn', async ($event) => {
      this.snack = true
      this.snackText = 'You have successfully withdrawn the request!'
      this.snackColor = 'green'
      this.arr_letters = []
      ;(this.pageLimit = 100), (this.skipCount = 0), this.getAllRequestsCount()
    })
  ...
  mounted() {
    this.getSelectedEmployee = this.user
    this.config_details = this.configData[0]
    if (
      this.configData &&
      this.configData[0].letterRequest &&
      this.configData[0].letterRequest.length > 0
    ) {
      this.request.letter_type =
        this.configData[0].letterRequest[0].letterDescription.requestType
      this.request.letter_sub_type =
        this.configData[0].letterRequest[0].letterDescription.requestSubType
    }
    this.resetClaimConfig()
    this.getUserInfo()
  ...
  beforeDestroy() {
    this.$nuxt.$off(
      'userSelected_letter',
      'resetLeaveForm_admin',
      'newLetterRequest',
      'refreshRequests',
      'addNewClaim',
      'newWfhRequestCreated',
      'letterUrl',
      'downloadLetterApprove',
      'requestWithdrawn'
    )
    this.breakloop = true
  },
  methods: {
    ...
    async downloadLetterPDF() {
      ...
      await this.$axios
        ...
        .then((response) => {
          ...
          link.setAttribute(
            'download',
            this.getUserName(data.user_id) + ' ' + data.letter_type + '.pdf'
    ...
    async getNewLetterPreview() {
      ...
      let getContent = function (localconfigData, localRequest) {
        for (
          let index = 0;
          index < localconfigData[0].letterRequest.length;
          index++
        ) {
          const data = localconfigData[0].letterRequest[index]
          if (data.letterDescription.requestSubType !== '') {
            if (
              localRequest.letter_type == data.letterDescription.requestType &&
              localRequest.letter_sub_type ==
                data.letterDescription.requestSubType
            )
              return data.contentbefore
          } else if (
            localRequest.letter_type == data.letterDescription.requestType
          )
            return data.contentbefore
      ...
      let funGetUserInfo = function (
        ...
        obj_user.user_name = bln_newLetter
          ? ''
          : obj_requestInfo.letter_fields.user_name
        obj_user.nationality = bln_newLetter
          ? obj_userInfo.personal.nationality
          : obj_requestInfo.letter_fields.nationality
        obj_user.first_name = bln_newLetter
          ? obj_userInfo.first_name
          : obj_requestInfo.letter_fields.first_name
        obj_user.last_name = bln_newLetter
          ? obj_userInfo.last_name
          : obj_requestInfo.letter_fields.last_name
        obj_user.passport_no = bln_newLetter
          ? obj_userInfo.documents.passport_number
          : obj_requestInfo.letter_fields.passport
        obj_user.emp_id = bln_newLetter
          ? obj_userInfo.emp_id
          : obj_requestInfo.letter_fields.emp_id
        obj_user.designation = bln_newLetter
          ? obj_userInfo.personal.designation
          : obj_requestInfo.letter_fields.role
        obj_user.department = bln_newLetter
          ? obj_userInfo.reporting.department
          : obj_requestInfo.letter_fields.department
        obj_user.date_of_joining = bln_newLetter
          ? obj_userInfo.date_of_joining
          : obj_requestInfo.letter_fields.work_start_date
        obj_user.bank_name = bln_newLetter
          ? obj_userInfo.bank.bank_name
          : obj_requestInfo.letter_fields.transfer_bank_name
        ...
        obj_user.department = bln_newLetter
          ? obj_userInfo.reporting.department
          : obj_requestInfo.letter_fields.department
        obj_user.iban = bln_newLetter
          ? obj_userInfo.bank.iban
          : obj_requestInfo.letter_fields.transfer_iban_number
        obj_user.total_fixed_salary = bln_newLetter
          ? obj_userInfo.salary.total_fixed ||
            obj_userInfo.salary.get('total_fixed')
          : obj_requestInfo.letter_fields.total_fixed
        obj_user.salary_currency = bln_newLetter
          ? obj_userInfo.bank.salary_currency
          : obj_userInfo.bank.salary_currency
        obj_user.accommodation_allowance = bln_newLetter
          ? obj_userInfo.salary.accommodation_allowance
          : obj_requestInfo.letter_fields.accommodation_allowance
        obj_user.medical_allowance = bln_newLetter
          ? obj_userInfo.salary.housing_allowance
          : obj_requestInfo.letter_fields.medical_allowance
        obj_user.transport_allowance = bln_newLetter
          ? obj_userInfo.salary.transport_allowance
          : obj_requestInfo.letter_fields.transport_allowance
        obj_user.other_allowance = bln_newLetter
          ? obj_userInfo.salary.other_allowance
          : obj_requestInfo.letter_fields.other_allowance
        obj_user.fixed_overtime = bln_newLetter
          ? obj_userInfo.salary.fixed_overtime
          : obj_requestInfo.letter_fields.fixed_overtime
        return obj_user
      }
      ...
      var company_id = bln_newLetter
        ? obj_userInfo.company_ID
        : obj_requestInfo.company_id
      var company_details = funGetCompanyDetails(company_id, arr_companies)
      var title = funGetUserTitle(
        obj_userInfo.personal.gender,
        obj_userInfo.personal.marital_status
      )
      var adjective = 'her'
      if (title == 'Mr.') {
        adjective = 'his'
      ...
      var salary_to_words = translator.toWords(user_data.total_fixed_salary)
      ...
      let linemanagerdetails = this.getUsersInfoForExcel(
        obj_userInfo.reporting.manager
      )
      sampleRequest.letter_fields['managername'] =
        funGetUserName(linemanagerdetails)
      sampleRequest.letter_fields['manageremail'] = linemanagerdetails.email
      sampleRequest.letter_fields['managerphone'] =
        linemanagerdetails.personal.phone
      sampleRequest.letter_fields['managerheshe'] =
        linemanagerdetails.personal.gender == 'Male' ? 'he' : 'she'
      sampleRequest.letter_fields['totalFixedWord'] = this.totalFixedWord
      sampleRequest.letter_fields['basicSalaryInWords'] =
        this.basicSalaryInWords
      ...
      sampleRequest.letter_fields['personalLoanTable2'] =
        this.personalLoanTable2
      sampleRequest.letter_fields['personalLoanTable3'] =
        this.personalLoanTable3
      ...
      sampleRequest.letter_fields['work_start_date'] = moment(
      ...
      sampleRequest.letter_fields['bankAccountNumber'] =
        user_data.account_number
      sampleRequest.letter_fields['totalFixed'] = parseFloat(
        user_data.total_fixed_salary
      ).toFixed(2)
      sampleRequest.letter_fields['salaryInWords'] =
        salary_to_words.charAt(0).toUpperCase() + salary_to_words.slice(1)
      ...
      sampleRequest.letter_fields['passportIssueDate'] = moment(
        obj_userInfo.documents.passport_issue
      ).format('D MMMM YYYY')
      sampleRequest.letter_fields['passportExpiryDate'] = moment(
        obj_userInfo.documents.passport_expiry
      ).format('D MMMM YYYY')
      sampleRequest.letter_fields['dob'] = moment(
        obj_userInfo.personal.dob
      ).format('D MMMM YYYY')
      ...
      sampleRequest.letter_fields['accommodation_allowance'] =
        user_data.accommodation_allowance
      sampleRequest.letter_fields['medical_allowance'] =
        user_data.medical_allowance
      sampleRequest.letter_fields['transport_allowance'] =
        user_data.transport_allowance
      ...
      await this.$axios
        .$post(
          process.env.documenturl + 'api/DocumentEditor/ReplaceContent',
          obj,
          {
            headers: {
              'Content-Type': 'application/json',
    ...
    replaceCustomKeys(userLetterInfo, letterInfo, bln_newLetter) {
      ...
      if (bln_newLetter) {
        if (
          userLetterInfo.personal.gender == 'Female' &&
          userLetterInfo.personal.marital_status == 'Married'
        )
          result = result.replaceAll('Ms.', 'Mrs.')
      } else {
        let title = userLetterInfo.hasOwnProperty('title')
          ? userLetterInfo.title
          : ''
      ...
      let first_name = bln_newLetter
        ? userLetterInfo.hasOwnProperty('first_name')
          ? userLetterInfo.first_name
          : ''
        : userLetterInfo.hasOwnProperty('first_name')
        ? userLetterInfo.first_name
        : ''
      ...
      let last_name = bln_newLetter
        ? userLetterInfo.hasOwnProperty('last_name')
          ? userLetterInfo.last_name
          : ''
        : userLetterInfo.hasOwnProperty('last_name')
        ? userLetterInfo.last_name
        : ''
      ...
      let emp_id = bln_newLetter
        ? userLetterInfo.hasOwnProperty('emp_id')
          ? userLetterInfo.emp_id
          : ''
        : userLetterInfo.hasOwnProperty('emp_id')
        ? userLetterInfo.emp_id
        : ''
      ...
      let address = bln_newLetter
        ? userLetterInfo.personal.hasOwnProperty('address')
          ? userLetterInfo.personal.address
          : ''
        : userLetterInfo.hasOwnProperty('address')
        ? userLetterInfo.address
        : ''
      ...
      let branch = bln_newLetter
        ? userLetterInfo.reporting.hasOwnProperty('branch')
          ? userLetterInfo.reporting.branch
          : ''
        : userLetterInfo.hasOwnProperty('branch')
        ? userLetterInfo.branch
        : ''
      ...
      let gender = bln_newLetter
        ? userLetterInfo.personal.hasOwnProperty('gender')
          ? userLetterInfo.personal.gender
          : ''
        : userLetterInfo.hasOwnProperty('gender')
        ? userLetterInfo.gender
        : ''
      ...
      if (bln_newLetter) {
        let loanAmount = 0
        // Total Fixed
        let total_compensation = userLetterInfo.salary.hasOwnProperty(
          'total_fixed'
        )
          ? userLetterInfo.salary.total_fixed
          : 0
        var t2w = translator.toWords(parseInt(total_compensation))
        var t2wFils = translator.toWords(
          parseInt(
            (parseFloat(total_compensation).toFixed(2) + '').split('.')[1]
          )
        )

        // Basic Salary
        let basicSalary = userLetterInfo.salary.hasOwnProperty('basic_salary')
          ? userLetterInfo.salary.basic_salary
          : 0
        var t2w_basicSalary = translator.toWords(parseInt(basicSalary))
        var t2wFils_basicSalary = translator.toWords(
          parseInt((parseFloat(basicSalary).toFixed(2) + '').split('.')[1])
        )

        if (
          this.request.letter_type == 'Salary Certificate' &&
          this.request.letter_sub_type == 'Car Loan'
        ) {
          loanAmount = this.request.letter_keys[1].hasOwnProperty('value')
            ? Number(this.request.letter_keys[1].value)
            : 0
        } else if (
          this.request.letter_type == 'Salary Certificate' &&
          this.request.letter_sub_type == 'Personal Loan'
        ) {
          loanAmount = this.request.letter_keys[2].hasOwnProperty('value')
            ? Number(this.request.letter_keys[2].value)
            : 0
        }

        // Loan Amount
        var t2w_loanAmount = translator.toWords(parseInt(loanAmount))
        var t2wFils_loanAmount = translator.toWords(
          parseInt((parseFloat(loanAmount).toFixed(2) + '').split('.')[1])
        )
        ...
        if (!isNaN(total_compensation)) {
          if (t2wFils.toString().toLowerCase() != 'zero') {
            total_fixed_word =
              t2w.replace(/(^\w{1})|(\s{1}\w{1}|-)/g, function (match) {
        ...
        total_fixed_word = userLetterInfo.hasOwnProperty('total_fixed_word')
          ? userLetterInfo.total_fixed_word
          : ''
        total_fixed_amount = userLetterInfo.hasOwnProperty('total_fixed')
          ? userLetterInfo.total_fixed
          : 0
        basic_salary_word = userLetterInfo.hasOwnProperty('basic_salary_word')
          ? userLetterInfo.basic_salary_word
          : ''
        basic_salary = userLetterInfo.hasOwnProperty('basic_salary')
          ? userLetterInfo.basic_salary
          : 0
        loan_amount_word = userLetterInfo.hasOwnProperty('loan_amount_word')
          ? userLetterInfo.loan_amount_word
          : 0
      ...
      result = result.replaceAll(
        '[empBasicSalary]',
        basic_salary.toLocaleString('en-US')
      )
      result = result.replaceAll(
        '[empTotalFixed]',
        total_fixed_amount.toLocaleString('en-US')
      )
      ...
      let acc_number = bln_newLetter
        ? userLetterInfo.bank.hasOwnProperty('account_number')
          ? userLetterInfo.bank.account_number
          : ''
        : userLetterInfo.hasOwnProperty('bank_account_number')
        ? userLetterInfo.bank_account_number
        : ''
      ...
      let compensation_table = bln_newLetter
        ...
        : userLetterInfo.hasOwnProperty('compensation_table')
        ? userLetterInfo.compensation_table
        : ''

      this.compensationTable = result.includes('compensationTable')
        ? this.computedSalaryTable
        : ''
      ...
      let personalLoan_table = bln_newLetter
        ...
        : userLetterInfo.hasOwnProperty('personalLoan_table')
        ? userLetterInfo.personalLoan_table
        : ''

      this.personalLoanTable = result.includes('personalLoanTable')
        ? this.computedPersonalLoanTable
        : ''
      ...
      let personalLoan_table2 = bln_newLetter
        ...
        : userLetterInfo.hasOwnProperty('personalLoan_table2')
        ? userLetterInfo.personalLoan_table2
        : ''

      this.personalLoanTable2 = result.includes('personalLoanTable2')
        ? this.computedPersonalLoanTable2
        : ''
      ...
      let personalLoan_table3 = bln_newLetter
        ...
        : userLetterInfo.hasOwnProperty('personalLoan_table3')
        ? userLetterInfo.personalLoan_table3
        : ''

      this.personalLoanTable3 = result.includes('personalLoanTable3')
        ? this.computedPersonalLoanTable3
        : ''
      ...
      let body = {
        ...
        endDate:
          this.selectedFilterDate == 'All'
            ? this.furDate.toISOString()
            : this.curDate.toISOString(),
        startDate: this.backDate.toISOString(),
      }

      try {
        let user_reqs = await this.$axios.$post(
          'requests/get_requests_users_info',
          body,
    ...
    async fetchDataForExcelDownload() {
      ...
      try {
        let user_letters = await this.$axios.$post(
    ...
    selectFilter(event) {
      if (event) this.selectedFilter = event
      // this.filterReqByUser = ''
      this.arr_letters = []
      this.skipCount = 0
      this.getAllRequests(false)
    ...
    getReqName(filterName) {
      ...
      if (filterName == 'loan') return 'Loan'
      if (filterName == 'education') return 'Education'
    ...
    requestFilterDownload(filterName) {
      ...
      if (filterName == 'loan') return 'Loan'
      if (filterName == 'education') return 'Education'
      if (filterName == 'passport') return 'Passport'
    ...
    changedDate(event) {
      if (event == 'All') {
        ;(this.curDate = new Date()), (this.arr_letters = [])
        this.skipCount = 0
    ...
    async getAllRequestsCount() {
      ...
      let body = {
        ...
        startDate: this.backDate.toISOString(),
        str_search_tag: '',
        req_user_id: this.user.id,
      }
      try {
        await this.$axios
          .$post('requests/get_all_requests_count/' + this.user.id, body)
          .then((res) => {
            ...
            this.educationCount = res.data.educationCount
              ? res.data.educationCount
              : 0
            this.passportCount = res.data.passportCount
              ? res.data.passportCount
              : 0
            this.totalCount =
              this.letterCount +
              this.wfhCount +
    ...
    async getAllRequests(filterSelected) {
      ...
      // backDate.setMonth( curDate.getMonth() - this.monthOld)
      ...
      try {
        ...
        let user_letters = await this.$axios.$post(url, body, {
          headers: { Authorization: AuthStr },
        })
        if (user_letters.data.results && user_letters.data.results.length > 0) {
    ...
    getFields() {
      ...
      } else if (this.selectedFilter == 'loan') {
        return this.json_fields_loan
      } else if (this.selectedFilter == 'education') {
        return this.json_fields_education
      } else {
    ...
    async getTeamLeaveList() {
      ...
      if (this.user) {
        await this.$axios
          .$get('/leaves/department/' + this.user.reporting.department, {
            headers: { Authorization: AuthStr },
          })
          .then((res) => {
    ...
    async getTeamWfhList() {
      ...
      if (this.user) {
        ...
        await this.$axios
          .$post('/wfh/dept', body, { headers: { Authorization: AuthStr } })
          .then((res) => {
            // console.log("wfh", res)
    ...
    getCalendarColor(val, type) {
      ...
      else if ((val == 'Processing' || val == 'processing') && type == 'leaves')
        return 'amber'
      else if ((val == 'Processing' || val == 'processing') && type == 'wfh')
        return '#ff5722'
      else if ((val == 'Completed' || val == 'completed') && type == 'leaves')
        return 'green'
      else if ((val == 'Completed' || val == 'completed') && type == 'wfh')
        return '#2196f3'
    ...
    isLetterRequest(data) {
      if (data.letterDescription.requestSubType !== '') {
        return (
          this.request.letter_type == data.letterDescription.requestType &&
          this.request.letter_sub_type == data.letterDescription.requestSubType
        )
      }
    ...
    getLetterTypeList() {
      ...
      let letterSubType = []
      if (this.configData[0]) {
        this.configData[0].letterRequest.forEach(function (letter) {
          if (letterType.indexOf(letter.letterDescription.requestType) == -1) {
            letterType.push(letter.letterDescription.requestType)
          }
          if (
            letter.letterDescription.requestSubType &&
            letterType.indexOf(letter.letterDescription.requestSubType) == -1
          ) {
            letterSubType.push(letter.letterDescription.requestSubType)
          }
        })
    ...
    loadHistory() {
      ...
      // this.monthOld = 6
      // this.backDate.setMonth( this.curDate.getMonth() - this.monthOld)
      // this.selectedFilterDate = 'last 6 month'
      this.getAllRequestsCount()
      // this.getAllRequests()
    },
    ...
    getTeamRequest(id) {
      return this.arr_letters.filter((a) => a.user_id == id)
    },
    getUserName(val) {
    ...
    getUserCompanyManager(letterType) {
      if (letterType.company_signatory) {
        let userCompany = this.companyData.filter(
          (ele) => ele._id == this.user.company_ID
        )
    ...
    computeRequestTypeList() {
      let requestTypeList = this.configData[0].requestTypes.filter(
        (a) => a != 'Attendance' && a != 'Salary Adjustment'
      )
      let arr = []
      if (requestTypeList) {
        for (let index = 0; index < requestTypeList.length; index++) {
          let obj = {}
          const element = requestTypeList[index]
          if (element == 'wfh') {
            obj.name = 'WFH'
            obj.value = 'wfh'
          } else if (element == 'Education Allowance') {
            obj.name = 'Education Allowance'
            obj.value = 'Education'
          } else {
            obj.name = element
            obj.value = element
          }
          if (element == 'Passport Release/Safekeep') {
            if (this.user.documents.passport_safekeep_required) {
              obj.name = 'Passport Release'
              obj.value = 'Passport Release'
              arr.push(obj)
            }
          } else {
            arr.push(obj)
    ...
    getDraftClaimReq() {
      if (this.computedServiceList.status?.toLowerCase() === 'draft') {
        ...
        this.request_claims.letter_fields =
          this.computedServiceList.letter_fields
        const claims = this.computedServiceList.claims
          ? this.computedServiceList.claims
          : []
        let letterSubTypes = []
        claims.forEach((claim) => {
          let subType = {}
          subType.letter_type = claim.letter_type
          let claimDetails = _.cloneDeep(
            allClaimSubTypes.find(
              (el) =>
                el.claimType === claim.letter_type &&
                el.claimSubType === claim.letter_sub_type
            )
          )
          if (claimDetails) {
            const claimKeys = []
            claimDetails.claim_keys.forEach((claimKey) => {
              ...
              if (key.type.toLowerCase() === 'attachments') {
                const file = claim.details.files.find(
                  (file) =>
                    this.getFormattedKeyName(file.name) ===
                    this.getFormattedKeyName(key.name)
                )
                if (file) {
                  key.value = []
                  key.value.push(file)
                ...
                key.value = claim.details[
                  'calculated_' + this.getFormattedKeyName(key.name)
                ]
                  ? claim.details[
                      'calculated_' + this.getFormattedKeyName(key.name)
                    ]
                  : claim.details[this.getFormattedKeyName(key.name)]
                  ? claim.details[this.getFormattedKeyName(key.name)]
                  : ''
                key.inputvalue = claim.details[
                  this.getFormattedKeyName(key.name)
                ]
                  ? claim.details[this.getFormattedKeyName(key.name)]
                  : ''
              } else {
                key.value = claim.details[this.getFormattedKeyName(key.name)]
                  ? claim.details[this.getFormattedKeyName(key.name)]
                  : ''
              }
              claimKeys.push(key)
    ...
    async deleteDraftClaimReq() {
      try {
        if (
          this.computedServiceList?.status?.toLowerCase() === 'draft' &&
          this.computedServiceList._id
        ) {
          const token = this.$store.getters.getToken
          const AuthStr = 'Bearer '.concat(token)
          const response = await this.$axios.$delete(
            '/claim/delete_draft/' + this.computedServiceList._id,
            {
              headers: {
  ...
  computed: {
    ...
    getInfoForDownload() {
      if (this.dateRangeTextExcel != '') {
        if (
          ...
          this.excel_downloaded_data == undefined ||
          this.excel_downloaded_data == null
        ) {
          return []
        } else {
          let fromDate = ''
          ...
          let abc = clonedData
          if (
            this.selectedFilter == 'letters' ||
            this.selectedFilter == 'claims' ||
            this.selectedFilter == 'attendance' ||
            this.selectedFilter == 'loan' ||
            ...
            this.selectedFilter == 'Leave' ||
            this.selectedFilter == 'wfh'
          ) {
            // if(this.date_calendar_download.length == 1){
            //     abc = clonedData.filter(a=>{
          ...
          if (this.selectedFilter == 'letters') {
            ...
            for (let index = 0; index < abc.length; index++) {
              ...
              lettersObj.emp_id = this.getUsersInfoForExcel(
                abc[index].user_id
              ).emp_id
              lettersObj.full_name =
                this.getUsersInfoForExcel(abc[index].user_id).first_name +
                ' ' +
                this.getUsersInfoForExcel(abc[index].user_id).middle_name +
                ' ' +
                this.getUsersInfoForExcel(abc[index].user_id).last_name
              lettersObj.doj = this.getUsersInfoForExcel(
                abc[index].user_id
              ).date_of_joining
              lettersObj.designation =
                this.getUsersInfoForExcel(abc[index].user_id) &&
                this.getUsersInfoForExcel(abc[index].user_id).personal
                  ? this.getUsersInfoForExcel(abc[index].user_id).personal
                      .designation
                  : ''
              lettersObj.dept =
                this.getUsersInfoForExcel(abc[index].user_id) &&
                this.getUsersInfoForExcel(abc[index].user_id).reporting
                  ? this.getUsersInfoForExcel(abc[index].user_id).reporting
                      .department
                  : ''
              lettersObj.company = this.getCompanyDetails(
                this.getUsersInfoForExcel(abc[index].user_id).company_ID
              )
              lettersObj.manager = this.getManagerDetails(
                this.getUsersInfoForExcel(abc[index].user_id).reporting
                  ? this.getUsersInfoForExcel(abc[index].user_id).reporting
                      .manager
                  : ''
              ...
              lettersObj.requestedDate = this.formatDate(
                abc[index].letter_fields.date
              )
              lettersObj.approvedDate = this.formatDate(
                abc[index].approvals.length > 0
                  ? abc[index].approvals[abc[index].approvals.length - 1]
                      .approved_date
                  : ''
              )
            ...
            for (let index = 0; index < abc.length; index++) {
              ...
              leaveObj.APPROVED_DATE = this.formatDate(
                abc[index].approvals.length > 0
                  ? abc[index].approvals[abc[index].approvals.length - 1]
                      .approved_date
                  : ''
              )
              leaveObj.COMAPNY = this.getCompanyDetails(
                this.getUsersInfoForExcel(abc[index].user_id).company_ID
              )
              leaveObj.DEPT =
                this.getUsersInfoForExcel(abc[index].user_id) &&
                this.getUsersInfoForExcel(abc[index].user_id).reporting
                  ? this.getUsersInfoForExcel(abc[index].user_id).reporting
                      .department
                  : ''
              leaveObj.DESIGNATION =
                this.getUsersInfoForExcel(abc[index].user_id) &&
                this.getUsersInfoForExcel(abc[index].user_id).personal
                  ? this.getUsersInfoForExcel(abc[index].user_id).personal
                      .designation
                  : ''
              leaveObj.DOJ = this.getUsersInfoForExcel(
                abc[index].user_id
              ).date_of_joining
              leaveObj.EMD_ID = this.getUsersInfoForExcel(
                abc[index].user_id
              ).emp_id
              leaveObj.FROM_DATE = this.formatDate(abc[index].from_date)
              leaveObj.LEAVE_TYPE = abc[index].leave_type
              leaveObj.MANAGER = this.getManagerDetails(
                this.getUsersInfoForExcel(abc[index].user_id).reporting
                  ? this.getUsersInfoForExcel(abc[index].user_id).reporting
                      .manager
                  : ''
            ...
            for (let index = 0; index < abc.length; index++) {
              ...
              if (claims?.length) {
                for (let i = 0; i < claims.length; i++) {
                  ...
                  claimsObj.emp_id = this.getUsersInfoForExcel(
                    abc[index].user_id
                  ).emp_id
                  claimsObj.first_name = this.getUsersInfoForExcel(
                    abc[index].user_id
                  ).first_name
                  claimsObj.middle_name = this.getUsersInfoForExcel(
                    abc[index].user_id
                  ).middle_name
                  claimsObj.last_name = this.getUsersInfoForExcel(
                    abc[index].user_id
                  ).last_name
                  claimsObj.gender =
                    this.getUsersInfoForExcel(abc[index].user_id) &&
                    this.getUsersInfoForExcel(abc[index].user_id).personal
                      ? this.getUsersInfoForExcel(abc[index].user_id).personal
                          .gender
                      : ''
                  claimsObj.marital_status =
                    this.getUsersInfoForExcel(abc[index].user_id) &&
                    this.getUsersInfoForExcel(abc[index].user_id).personal
                      ? this.getUsersInfoForExcel(abc[index].user_id).personal
                          .marital_status
                      : ''
                  claimsObj.doj = this.getUsersInfoForExcel(
                    abc[index].user_id
                  ).date_of_joining
                  claimsObj.designation =
                    this.getUsersInfoForExcel(abc[index].user_id) &&
                    this.getUsersInfoForExcel(abc[index].user_id).personal
                      ? this.getUsersInfoForExcel(abc[index].user_id).personal
                          .designation
                  ...
                  claimsObj.dept =
                    ...
                    this.getUsersInfoForExcel(abc[index].user_id).reporting
                      ? this.getUsersInfoForExcel(abc[index].user_id).reporting
                          .department
                      : ''
                  claimsObj.company = this.getCompanyDetails(
                    this.getUsersInfoForExcel(abc[index].user_id).company_ID
                  )
                  claimsObj.manager = this.getManagerDetails(
                    this.getUsersInfoForExcel(abc[index].user_id).reporting
                      ? this.getUsersInfoForExcel(abc[index].user_id).reporting
                          .manager
                      : ''
                  )
                  claimsObj.reference_number =
                    abc[index].letter_fields.reference_number
                  ...
                  claimsObj.approvedDate = this.formatDate(
                    abc[index].approvals.length > 0
                      ? abc[index].approvals[abc[index].approvals.length - 1]
                          .approved_date
                      : ''
            ...
            for (let index = 0; index < abc.length; index++) {
              ...
              loanObj.emp_id = this.getUsersInfoForExcel(
                abc[index].user_id
              ).emp_id
              loanObj.first_name = this.getUsersInfoForExcel(
                abc[index].user_id
              ).first_name
              loanObj.middle_name = this.getUsersInfoForExcel(
                abc[index].user_id
              ).middle_name
              loanObj.last_name = this.getUsersInfoForExcel(
                abc[index].user_id
              ).last_name
              loanObj.gender =
                this.getUsersInfoForExcel(abc[index].user_id) &&
                this.getUsersInfoForExcel(abc[index].user_id).personal
                  ? this.getUsersInfoForExcel(abc[index].user_id).personal
                      .gender
                  : ''
              loanObj.marital_status =
                this.getUsersInfoForExcel(abc[index].user_id) &&
                this.getUsersInfoForExcel(abc[index].user_id).personal
                  ? this.getUsersInfoForExcel(abc[index].user_id).personal
                      .marital_status
                  : ''
              loanObj.doj = this.getUsersInfoForExcel(
                abc[index].user_id
              ).date_of_joining
              loanObj.designation =
                this.getUsersInfoForExcel(abc[index].user_id) &&
              ...
              loanObj.dept =
                ...
                this.getUsersInfoForExcel(abc[index].user_id).reporting
                  ? this.getUsersInfoForExcel(abc[index].user_id).reporting
                      .department
                  : ''
              loanObj.company = this.getCompanyDetails(
                this.getUsersInfoForExcel(abc[index].user_id).company_ID
              )
              loanObj.manager = this.getManagerDetails(
                this.getUsersInfoForExcel(abc[index].user_id).reporting
                  ? this.getUsersInfoForExcel(abc[index].user_id).reporting
                      .manager
                  : ''
              )
              loanObj.loan_type = abc[index].letter_fields.loan_type
              loanObj.loan_amount = abc[index].letter_fields.loan_amount
              loanObj.monthly_payment = abc[index].letter_fields.monthly_payment
              loanObj.loan_months = abc[index].letter_fields.loan_months
              loanObj.comments = abc[index].letter_fields.description
              loanObj.requestedDate = this.formatDate(
                abc[index].letter_fields.date
              )
              loanObj.approvedDate = this.formatDate(
                abc[index].approvals.length > 0
                  ? abc[index].approvals[abc[index].approvals.length - 1]
                      .approved_date
                  : ''
            ...
            for (let index = 0; index < abc.length; index++) {
              ...
              attendanceObj.emp_id = this.getUsersInfoForExcel(
                abc[index].user_id
              ).emp_id
              attendanceObj.first_name = this.getUsersInfoForExcel(
                abc[index].user_id
              ).first_name
              attendanceObj.middle_name = this.getUsersInfoForExcel(
                abc[index].user_id
              ).middle_name
              attendanceObj.last_name = this.getUsersInfoForExcel(
                abc[index].user_id
              ).last_name
              attendanceObj.gender =
                this.getUsersInfoForExcel(abc[index].user_id) &&
                this.getUsersInfoForExcel(abc[index].user_id).personal
                  ? this.getUsersInfoForExcel(abc[index].user_id).personal
                      .gender
                  : ''
              attendanceObj.marital_status =
                this.getUsersInfoForExcel(abc[index].user_id) &&
                this.getUsersInfoForExcel(abc[index].user_id).personal
                  ? this.getUsersInfoForExcel(abc[index].user_id).personal
                      .marital_status
                  : ''
              attendanceObj.doj = this.getUsersInfoForExcel(abc[index].user_id)
                ? this.getUsersInfoForExcel(abc[index].user_id).date_of_joining
                : ''
              attendanceObj.designation =
                this.getUsersInfoForExcel(abc[index].user_id) &&
                this.getUsersInfoForExcel(abc[index].user_id).personal
                  ? this.getUsersInfoForExcel(abc[index].user_id).personal
                      .designation
              ...
              attendanceObj.dept =
                ...
                this.getUsersInfoForExcel(abc[index].user_id).reporting
                  ? this.getUsersInfoForExcel(abc[index].user_id).reporting
                      .department
                  : ''
              attendanceObj.company = this.getCompanyDetails(
                this.getUsersInfoForExcel(abc[index].user_id).company_ID
              )
              attendanceObj.manager = this.getManagerDetails(
                this.getUsersInfoForExcel(abc[index].user_id).reporting
                  ? this.getUsersInfoForExcel(abc[index].user_id).reporting
                      .manager
                  : ''
              ...
              attendanceObj.login_date = dateFormat(
                abc[index].letter_fields.startTime
              )
              attendanceObj.login_time = timeFormat(
                abc[index].letter_fields.startTime
              )
              attendanceObj.logout_date = dateFormat(
                abc[index].letter_fields.endTime
              )
              attendanceObj.logout_time = timeFormat(
                abc[index].letter_fields.endTime
              )
              attendanceObj.user_comments = abc[index].letter_fields.comments
              attendanceObj.status = abc[index].status
              attendanceObj.requestCreationDate = new Date(
                abc[index].date_created
              ).toLocaleString()
            ...
            for (let index = 0; index < abc.length; index++) {
              ...
              wfhObj.emp_id = this.getUsersInfoForExcel(
                abc[index].user_id
              ).emp_id
              wfhObj.full_name =
                this.getUsersInfoForExcel(abc[index].user_id).first_name +
                ...
                ' ' +
                this.getUsersInfoForExcel(abc[index].user_id).last_name
              wfhObj.doj = this.getUsersInfoForExcel(
                abc[index].user_id
              ).date_of_joining
              wfhObj.designation =
                this.getUsersInfoForExcel(abc[index].user_id) &&
                this.getUsersInfoForExcel(abc[index].user_id).personal
                  ? this.getUsersInfoForExcel(abc[index].user_id).personal
                      .designation
                  : ''
              wfhObj.manager = this.getManagerDetails(
                this.getUsersInfoForExcel(abc[index].user_id).reporting
                  ? this.getUsersInfoForExcel(abc[index].user_id).reporting
                      .manager
                  : ''
              ...
              wfhObj.dept =
                ...
                this.getUsersInfoForExcel(abc[index].user_id).reporting
                  ? this.getUsersInfoForExcel(abc[index].user_id).reporting
                      .department
                  : ''
              wfhObj.company = this.getCompanyDetails(
                this.getUsersInfoForExcel(abc[index].user_id).company_ID
              )
              ...
              wfhObj.approvedDate = this.formatDate(
                abc[index].approvals.length > 0
                  ? abc[index].approvals[abc[index].approvals.length - 1]
                      .approved_date
                  : ''
            ...
            for (let index = 0; index < abc.length; index++) {
              ...
              allObj.company = this.getCompanyDetails(
                abc[index].user_data.company_ID
              )
              allObj.manager = this.getManagerDetails(
                abc[index].user_data.manager || ''
              )
              ...
              allObj.date_created =
                this.formatDate(abc[index].from_date) ||
                this.formatDate(abc[index].date_created)

              allObj.to_date = this.formatDate(abc[index].to_date)
              allObj.letter_type =
                abc[index].request_type == 'letters'
                  ? abc[index].letter_type
                  : ''
              allObj.letter_sub_type =
                abc[index].request_type == 'letters'
                  ? abc[index].letter_sub_type
                  : ''
              allObj.leave_type =
                abc[index].request_type == 'leaves' ? abc[index].leave_type : ''
              allObj.amount = abc[index].hasOwnProperty('letter_fields')
                ? abc[index].letter_fields.amount
                : ''
              allObj.description =
                abc[index].request_type == 'attendance'
                  ? abc[index].letter_fields.comments
                  : abc[index].hasOwnProperty('letter_fields')
                  ? abc[index].letter_fields.description
                  : ''
              allObj.startTime =
                abc[index].request_type == 'attendance'
                  ? moment(String(abc[index].letter_fields.startTime)).format(
                      'DD MMM, YYYY'
                    ) +
                    ' ' +
                    moment(String(abc[index].letter_fields.startTime)).format(
              ...
              allObj.endTime =
                abc[index].request_type == 'attendance'
                  ? moment(String(abc[index].letter_fields.endTime)).format(
                      'DD MMM, YYYY'
                    ) +
                    ' ' +
                    moment(String(abc[index].letter_fields.endTime)).format(
              ...
              if (
                abc[index].request_type == 'claims' &&
                abc[index].claims?.length
              ) {
                const claims = abc[index].claims
                for (let i = 0; i < claims.length; i++) {
                  allObj.reference_number =
                    abc[index].letter_fields.reference_number
                  allObj.claim_type = claims[i].letter_type
                    ? claims[i].letter_type
                    : ''
                  allObj.claim_sub_type = claims[i].letter_sub_type
                    ? claims[i].letter_sub_type
                    : ''
                  arr.push({
                    ...allObj,
                  })
    ...
    dateRangeText() {
      ...
      if (this.date_calendar.length == 1) {
        return `${formatDate(this.date_calendar[0])} - ${formatDate(
          this.date_calendar[0]
        )}`
    ...
    dateRangeTextExcel() {
      ...
      if (this.date_calendar_download.length == 1) {
        return `${formatDate(this.date_calendar_download[0])} - ${formatDate(
          this.date_calendar_download[0]
        )}`
      } else if (this.date_calendar_download.length > 1) {
    ...
    computeDateNewRequest() {
      return moment(new Date().toISOString().substr(0, 10)).format(
        this.filterLetterFormats.dateFormat
      )
    },
    replaceHeader() {
      this.request.letter_keys = this.filterLetterFormats.letterKeys
      ...
      let Addressee = Letter.computeLetterAddressee(
        userInfo,
        letter_template,
        requestInfo,
        bln_newLetter,
    ...
    inlineStyle() {
      return {
        backgroundImage: `url(${
          this.getCompanyInformation.letterDetail
            ? this.getCompanyInformation.letterDetail.waterMarkLink
            : ''
        })`,
      }
    },
    leftSidebarLink() {
      return {
        backgroundImage: `url(${
          this.getCompanyInformation.letterDetail
            ? this.getCompanyInformation.letterDetail.leftSideBarLink
            : ''
        })`,
      }
    },
    rightSidebarLink() {
      return {
        backgroundImage: `url(${
          this.getCompanyInformation.letterDetail
            ? this.getCompanyInformation.letterDetail.rightSideBarLink
            : ''
        })`,
      }
    },
    filterLetterFormats() {
      ...
      if (
        ...
        this.request.letter_sub_type = 'Travel'
    ...
    replaceLetterkey() {
      ...
      let bln_newLetter = true
      const Letter = new Letters()
      let LetterContent = Letter.computeLetterContent(
        userInfo,
        letter_template,
        requestInfo,
        bln_newLetter,
    ...
    computeLetterPDFStyles() {
      if (this.filterLetterFormats) {
        ...
        this.obj_letter_preview_styles.leftSidebarCol =
          letterFomat.leftSidebarCol
        this.obj_letter_preview_styles.bodyCol = letterFomat.bodyCol
        this.obj_letter_preview_styles.rightSidebarCol =
          letterFomat.rightSidebarCol
        this.obj_letter_pdf_styles.header =
          letterFomat.headerShow && letterFomat.headerShow.pdf
        this.obj_letter_pdf_styles.date_format = letterFomat.dateFormat
        this.obj_letter_pdf_styles.date_position =
          letterFomat.formattedText.formatDate
        this.obj_letter_pdf_styles.addressee_position =
          letterFomat.formattedText.formatHeader
        this.obj_letter_pdf_styles.subject_position =
          letterFomat.formattedText.formatStyle
        this.obj_letter_pdf_styles.left_sideBar =
          letterFomat.leftSideBar && letterFomat.leftSideBar.pdf
        this.obj_letter_pdf_styles.right_sideBar =
          letterFomat.rightSideBar && letterFomat.rightSideBar.pdf
        this.obj_letter_pdf_styles.signature =
          letterFomat.signatureShow && letterFomat.signatureShow.pdf
        this.obj_letter_pdf_styles.stamp =
          letterFomat.stampShow && letterFomat.stampShow.pdf
    ...
    computeLetterPreviewStyles() {
      ...
      this.obj_letter_preview_styles.date_position =
        letterFomat.formattedText.formatDate
      this.obj_letter_preview_styles.addressee_position =
        letterFomat.formattedText.formatHeader
      this.obj_letter_preview_styles.subject_position =
        letterFomat.formattedText.formatStyle
      this.obj_letter_preview_styles.left_sideBar =
        letterFomat.leftSideBar.preview
      this.obj_letter_preview_styles.right_sideBar =
        letterFomat.rightSideBar.preview
      this.obj_letter_preview_styles.signature =
        letterFomat.signatureShow.preview
      this.obj_letter_preview_styles.stamp = letterFomat.stampShow.preview
      this.obj_letter_preview_styles.watermark =
        letterFomat.watermarkShow.preview
    ...
    getCompanyInformation() {
      let company = this.companyData.filter(
        (a) => a._id == this.user.company_id
      )
      if (this.selectedEmployeeId != '') {
        company = this.companyData.filter((a) => a._id == this.user.company_id)
      }
    ...
    visibleData() {
      ...
      var getUserName = function (val) {
        if (val == null || val == undefined || val == '') {
          ...
          return abc.length > 0
            ? abc[0].first_name + ' ' + abc[0].last_name
            : ''
        }
      }
    ...
    isDraftClaimRequest() {
      return this.computedServiceList?.status.toLowerCase() === 'draft'
    },
  },
}
</script>
