    ...
    <div class="pt-0">
        <v-row>
            <v-col cols="12" sm="12" md="12" lg="11">
                <h2 class="darkBlue-heading-text subHeadingFontSize">Email Templates</h2>
            </v-col>
        </v-row>
        <v-row>
            ...
            <v-col cols="12" sm="12" md="8">
                <v-row v-if="emailloading" style="min-height:100%;align-items:center;justify-content:center;">
                    <v-col cols="auto">
                        <v-img src="/animated/refresh.svg" max-width="fit-content" height="200" contain
                            class="mr-3"></v-img>
                    </v-col>
                ...
                <v-card class="pa-5 rounded-xl" height="800" style="overflow: hidden" v-else>
                    <v-card-title class="px-3 tw-sticky tw-top-0 tw-z-10 tw-bg-white">
                        <v-row>
                            <v-col cols="12" sm="6">
                                <span class="px-0 pt-1 darkBlue-heading-text subHeadingFontSize">{{ emailedit ? 'Edit ' :
                                    '' }}{{ selectedEmail.name }}</span>
                            </v-col>
                            <v-col cols="12" sm="6" class="text-right pt-0">
                                <v-btn elevation="'0" width="150px" color="primary" class="border-radius-medium"
                                    ...
                                    @click="saveEmailTemplate(selectedEmail)">Update</v-btn>
                                ...
                                <v-btn elevation="'0" width="150px" class="border-radius-medium" v-if="emailedit"
                                    @click="loadServiceList(selectedEmail); emailedit = !emailedit">Cancel</v-btn>
                                <v-btn color="#5C7EEF" v-else-if="!empdetEdit" class="rounded-xl" small outlined
                                    @click="emailedit = true">Edit</v-btn>
                                <v-btn color="red" class="rounded-xl" small outlined
                                    @click="deleteEmail = true">Delete</v-btn>
                            </v-col>
                        </v-row>
                    </v-card-title>
                    <v-divider class="mt-0"></v-divider>
                    <v-row>
                        <v-col cols="3">
                            ...
                            <span v-if="!emailedit"
                                class="darkBlue-heading-text font-weight-normal textFontSize pa-0 ma-0">{{
                                    selectedEmail.flag }}</span>
                            <v-select v-model="selectedEmail.flag"
                        ...
                        <v-col cols="3">
                            ...
                            <span class="darkBlue-heading-text font-weight-normal textFontSize pa-0 ma-0">{{
                                selectedEmail.module }}</span>
                        </v-col>
                        <v-col cols="6">
                            <p class="grey-heading-text font-weight-medium textFontSize pa-0 ma-0">Name</p>
                            <span class="darkBlue-heading-text font-weight-normal textFontSize pa-0 ma-0"
                                v-if="!emailedit">{{ selectedEmail.name }}</span>
                            <v-text-field v-model="selectedEmail.name" :rules="genericRule" v-else></v-text-field>
                        </v-col>
                        <v-col cols="6">
                        ...
                        <v-col cols="12">
                            ...
                            <span class="darkBlue-heading-text font-weight-normal textFontSize pa-0 ma-0"
                                v-if="!emailedit">{{ selectedEmail.subject }}</span>
                            <v-text-field v-model="selectedEmail.subject" :rules="genericRule" v-else></v-text-field>
                        </v-col>
                    </v-row>
                    <v-divider class="mt-0"></v-divider>
                    ...
                    <v-tabs-items v-model="tab">
                        <v-tab-item id="preview" class="">
                            <div class="pt-0 overflow-y-auto" style="max-height: 400px">
                                <div v-if="selectedEmail && !emailedit" v-html="selectedEmail.content"
                                    :style="{ 'height': '340px' }"></div>
                                <client-only v-if="selectedEmail && emailedit">
                                    <Modifyemail :body="selectedEmail.content" :dynamicVariables="dynamicVariables"
                                        @input="updateContent" />
                                </client-only>
                        ...
                        <v-tab-item id="rkeys" class="">
                            <div class="pt-0 overflow-y-auto" style="max-height: 270px">
                                <client-only v-if="selectedEmail">
                                    <ModifyReplaceKeys :keys="selectedEmail.auto_replace_keys"
                                        :module="selectedEmail.module" :edit="emailedit"
                        ...
                        <v-tab-item id="ikeys" class="">
                            <div class="pt-0 overflow-y-auto" style="max-height: 270px">
                                <client-only v-if="selectedEmail">
                                    <ModifyInputKeys :keys="selectedEmail.user_input_keys" :module="selectedEmail.module"
        ...
        <div v-if="deleteEmail" class="tw-fixed tw-inset-0 tw-z-50 tw-flex tw-items-center tw-justify-center tw-bg-black/40 tw-backdrop-blur-sm tw-transition-all tw-duration-300">
          <div class="tw-bg-white tw-rounded-2xl tw-shadow-2xl tw-border tw-border-red-100 tw-px-8 tw-py-10 tw-max-w-lg tw-w-full tw-relative tw-animate-fadeIn">
            <div class="tw-flex tw-flex-col tw-items-center tw-mb-6">
              ...
              <h3 class="tw-text-xl tw-font-bold tw-text-gray-900 tw-mb-1">Delete Email Template?</h3>
              <p class="tw-text-gray-600 tw-text-center tw-mb-2 tw-max-w-xs">This action cannot be undone. This will permanently delete the email template and all its data.</p>
            </div>
            <div class="tw-flex tw-justify-end tw-gap-4">
              <button @click="deleteEmail = false" class="tw-px-6 tw-py-2 tw-rounded-lg tw-bg-gray-100 tw-text-gray-700 tw-font-semibold tw-border tw-border-gray-200 tw-shadow-sm tw-hover:bg-gray-200 tw-transition-all">Cancel</button>
              <button @click="handleEmailDelete" class="tw-px-6 tw-py-2 tw-rounded-lg tw-bg-red-500 tw-text-white tw-font-semibold tw-shadow-md tw-border tw-border-red-400 tw-hover:bg-red-600 tw-transition-all">Delete</button>
            </div>
          </div>
        </div>
            ...
            <v-card class="rounded-xl pa-0 pt-0" flat min-height="230">
                    ...
                    <v-row>
                        <v-col cols="10">
                            <v-img src="/shift/build.svg" max-width="fit-content" height="fit-content" class="mr-2"
                                contain></v-img>
                            <span class="darkBlue-heading-text font-weight-normal subHeadingFontSize">New Email</span>
                        </v-col>
                ...
                <v-card-text class="py-0">
                    <v-form ref="newEmail">
                                ...
                                <v-text-field v-model="toEmails" :rules="genericRule">
                                    <template v-slot:append>
                                        <v-tooltip top>
                                            ...
                                            <span>Email IDs Separated by comma</span><br>
                                            <span>Note: Note: Use Module name if email should be triggered to connected
                                                module</span>
                                        </v-tooltip>
                                    </template>
                                </v-text-field>
                        ...
                        <v-row class="pb-0">
                            <v-col cols="6 mt-5">
                                <span class="grey-heading-text textFontSize font-weight-medium">Template Visibility</span>
                            </v-col>
                            <v-col cols="6">
                                <v-autocomplete :items="emailTemplateVisibility" v-model="newEmail.visibility" :rules="genericRule">
                                </v-autocomplete>
                            </v-col>
                        </v-row>
...
export default {
    ...
    data() {
        return {
            ...
            selectedEmail: {},
            system_modules: [],
            emailTemplateVisibility:['public', 'private'],
            newEmail: {
                module: '',
                to: [],
                cc: [],
    ...
    async mounted() {
        ...
        this.loadServiceList(this.emaillist[0])
    },
    methods: {
        getClass(val) {
        ...
        async saveEmailTemplate() {
            ...
            let keysArrays = this.selectedEmail.auto_replace_keys.map(a => a.key).concat(this.selectedEmail.user_input_keys.map(a => a.key))
            ...
            if (findDuplicates(keysArrays).length > 0) {
                ...
                this.selectedEmail.to = this.toEmails.split(',')
                this.selectedEmail.cc = this.ccEmails.split(',')
                const AuthStr = 'Bearer '.concat(this.$store.state.token)
                await await this.$axios.patch(`/email_template/${this.selectedEmail.id}`, this.selectedEmail, { headers: { Authorization: AuthStr } })
                    .then(async (response) => {
                        let index = this.findIndex(this.emaillist, 'id', this.selectedEmail.id)
                        this.emaillist[index].flag = this.selectedEmail.flag
                        this.emaillist[index].name = this.selectedEmail.name
                        console.log(this.emaillist[index])
                        this.emailedit = false
                        this.newEmailLoading = false
                        this.toEmails = ''
                        this.ccEmails = ''
                        await this.loadServiceList(this.selectedEmail)
        ...
        async handleEmailDelete() {
            const AuthStr = 'Bearer '.concat(this.$store.state.token)
            await  this.$axios.delete(`/email_template/${this.selectedEmail.id}`, { headers: { Authorization: AuthStr } })
                    .then(async (response) => {
                        await this.getallemails()
                        this.loadServiceList(this.emaillist[0])
        ...
        async addNewEmail() {
            ...
            if (this.$refs.newEmail.validate()) {
                ...
                const AuthStr = 'Bearer '.concat(this.$store.state.token)
                await this.$axios.post(`/email_template/add/`, this.newEmail, { headers: { Authorization: AuthStr } })
                    .then(async (response) => {
                        this.emailedit = false
                        this.newEmail = {
                            module: '',
        ...
        getDynamicVariables() {
            this.dynamicVariables = this.selectedEmail.auto_replace_keys.map((a) => { return { 'key': a.key, 'label': a.label } }).concat(this.selectedEmail.user_input_keys.map((a) => { return { 'key': a.key, 'label': a.label } }))
        },
        updateReplaceKeys(event) {
            this.selectedEmail.auto_replace_keys = event.keys
            this.getDynamicVariables()
        },
        updateInputKeys(event) {
            this.selectedEmail.user_input_keys = event.keys
            this.getDynamicVariables()
        },
        updateContent(event) {
            this.selectedEmail.content = event
        },
        async loadServiceList(data) {
            this.emailloading = true
            const AuthStr = 'Bearer '.concat(this.$store.state.token)
            this.$axios.get(`/email_template/id/${data.id}`, { headers: { Authorization: AuthStr } })
                .then((response) => {

                    this.emailloading = false
                    this.selectedEmail = response.data
                    this.toEmails = this.selectedEmail.to.join(",")
                    this.ccEmails = this.selectedEmail.cc.join(",")
                    this.getDynamicVariables()
                })
                .catch(e => console.log(e))
        },
    ...
    computed: {
        visiblePages() {
            let data = _.cloneDeep(this.emaillist)
            let returnData = []
            if (this.searchQuery) {
