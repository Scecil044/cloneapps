        ...
        <div class="default-section">
          <div ref="de_titlebar" id="documenteditor_titlebar" class="e-de-ctn-title">
            <div v-on:keydown="titleBarKeydownEvent" v-on:click="titleBarClickEvent" class="single-line" id="documenteditor_title_contentEditor" title="Document Name. Click or tap to rename this document." contenteditable="false">
                <label v-if="showdocumentname" v-on:blur="titleBarBlurEvent" id="documenteditor_title_name" :style="titileStyle" >{{documentName}}</label>
            </div>    
            <v-progress-circular v-if="downloadProgress" indeterminate  :style="iconStyle" small color="white" size="20"  class="mr-3" />
            <ejs-button  v-else-if="showdownloadbutton"  id="de-print" :style="iconStyle" :iconCss="exportIconCss" v-on:click.native="printDwnClick" title="Save Document">Download PDF</ejs-button>
            <ejs-button  v-else-if="showclosebutton" id="de-close" :style="iconStyle" v-on:click.native="printClsClick" title="Save Document">Close</ejs-button>	
        </div>
        <ejs-documenteditorcontainer ref="doceditcontainer" :height='height'></ejs-documenteditorcontainer>            
        </div>
    </div>
</div>
</template>
...
export default Vue.extend({
    ...
    // props:['content','documentName','_id','showdownloadbutton','showclosebutton','showdocumentname','height'],
  components: {
    },
    data: function() {
        return {
    ...
    provide:{
      ...
      methods: {
        ...
        printBtnClick: function () {
            this.$emit('save',{_id:this._id,documentName:this.documentName,content:this.$refs.doceditcontainer.ej2Instances.documentEditor.serialize()})
        ...
        printDwnClick: async function () {
            ...
            // this.$emit('save',{documentName:this.$refs.doceditcontainer.ej2Instances.documentEditor.documentName,content:this.$refs.doceditcontainer.ej2Instances.documentEditor.serialize()})
            // this.$refs.doceditcontainer.ej2Instances.documentEditor.print();
        },
    ...
    mounted() {
        this.$nextTick(function () {
          ...
          this.$refs.doceditcontainer.ej2Instances.restrictEditing  = true
